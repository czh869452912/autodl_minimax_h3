...
- 后续进展（2026-09-10，未发布）：已实现损坏、设备编码不支持和探测失败的独立分类与提示，阻止不兼容文件的无效自动重下载；已实现Android按需软件解码生成兼容副本，并将原件和副本独立保存到相册；模拟器验证通过，真机验收待完成。方案和边界见 [客户端视频编码兼容性评估](media-codec-compatibility.md)。
- 确认日期：2026-09-09。
