...
令牌是账户凭据。此版本只在内存中使用令牌，不会把令牌写入 APK 或本机任务历史；
请勿把自己的令牌提交到 Git 仓库或分享给他人。由于图片和音频采用 Base64 直接随
请求发送，单次选择的文件总量限制为 50 MB，较大的素材建议先压缩或使用可访问的
HTTPS 文件 URL。
