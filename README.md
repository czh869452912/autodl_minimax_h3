...
- **手动同步工作流（1.4.17）**：设置页从官方签名仓库同步已兼容的 AutoDL H3 工作流。创建页和 Prompt 助手导入预览可选择工作流，按目标 schema 对齐参数和素材约束。新增的升级画质 `zm-u24` 包作为远端发布源提供；首次发布前仍使用内置工作流。发布方法见 [工作流仓库发布指南](docs/workflow-registry-publishing.md)。
