...
function trimTrailingSlashes$2(path) {
	let end = path.length;
...
var StateBackend = class {
	...
	*/
	delete(filePath) {
		const files = this.files;
		const base = trimTrailingSlashes$2(filePath) || "/";
		const prefix = base === "/" ? "/" : `${base}/`;
		const paths = Object.keys(files).filter((path) => path === base || path.startsWith(prefix));
		if (paths.length === 0) return { error: `Error: File '${filePath}' not found` };
		const update = Object.fromEntries(paths.map((path) => [path, null]));
		if (!this.isLegacy) {
			this.sendFilesUpdate(update);
			return { path: filePath };
		}
...
var CompositeBackend = class {
	...
	*/
	getBackendAndKey(key) {
		for (const [prefix, backend] of this.sortedRoutes) if (key === prefix.slice(0, -1) || key.startsWith(prefix)) {
	...
	*/
	prefixDeleteFilesUpdate(filesUpdate, routePrefix) {
		const routeRoot = routePrefix.slice(0, -1);
		return Object.fromEntries(Object.keys(filesUpdate).map((path) => [routeRoot + path, null]));
	}
	/**
	* Restore composite paths in a deletion result from a single backend.
	*/
	restoreDeleteResult(result, filePath, routePrefix) {
		const restored = { ...result };
		if (result.path !== void 0) restored.path = filePath;
		if (routePrefix && result.filesUpdate) restored.filesUpdate = this.prefixDeleteFilesUpdate(result.filesUpdate, routePrefix);
		return restored;
	}
	/**
	...
	*/
	async delete(filePath) {
		...
		const baseRoute = this.sortedRoutes.find(([routePrefix]) => this.isPathWithinRoute(filePath, routePrefix))?.[0];
		const targets = [{
			backend: baseBackend,
			key: baseKey,
			routePrefix: baseRoute
		}];
		for (const [routePrefix, backend] of this.sortedRoutes) if (routePrefix !== baseRoute && this.isRouteUnderPath(routePrefix, filePath)) targets.push({
			backend,
			key: "/",
			routePrefix
		});
		for (const target of targets) if (!target.backend.delete) return { error: `Error: deletion is not available for '${filePath}' on ${target.routePrefix ? `mounted route '${target.routePrefix}'` : "default backend"}.` };
		if (targets.length === 1) {
		...
		for (const target of targets) {
			...
			try {
				...
				return { error: `Error deleting '${filePath}': ${typeof error === "object" && error !== null && "message" in error && typeof error.message === "string" ? error.message : String(error)}. Deletion may be partial; ${completed} earlier backend(s) completed and remaining backends were not attempted.` };
			}
			if (result.error) {
				...
				return { error: `Error deleting '${filePath}': ${result.error}. Deletion may be partial; ${completed} earlier backend(s) completed and remaining backends were not attempted.` };
			}
			completed += 1;
			if (result.filesUpdate === null) hasNullFilesUpdate = true;
			else if (result.filesUpdate) {
				hasFilesUpdate = true;
				Object.assign(filesUpdate, target.routePrefix ? this.prefixDeleteFilesUpdate(result.filesUpdate, target.routePrefix) : result.filesUpdate);
			}
		}
		if (completed === 0) return firstNotFound ?? { error: `Error: File '${filePath}' not found` };
		if (hasFilesUpdate) return {
			path: filePath,
			filesUpdate
		};
		if (hasNullFilesUpdate) return {
			path: filePath,
			filesUpdate: null
		};
		return { path: filePath };
...
function hasGlobMetaCharacter(pattern) {
	return GLOB_WILDCARD_CHARACTERS.some((character) => pattern.includes(character));
}
/**
...
*/
function isRelativeTo(child, ancestor) {
...
*/
function globAnchor(pattern) {
	const safe = [];
	for (const part of posixParts(pattern)) {
		if (GLOB_WILDCARD_CHARACTERS.some((character) => part.includes(character))) break;
		safe.push(part);
	}
...
*/
function pathsOverlap(callPath, ruleAnchor) {
...
*/
function wildcardDeleteOverlap(pattern, anchor, target) {
	if (anchor === "/") return true;
	if (globMatch(target, pattern)) return true;
	if (isRelativeTo(anchor, target)) return true;
	if (!isRelativeTo(target, anchor)) return false;
	const anchorParts = posixParts(anchor);
	const suffix = posixParts(pattern).slice(anchorParts.length);
	if (suffix.length !== 1 || suffix[0].includes("**")) return true;
	const targetParts = posixParts(target);
	for (let depth = anchorParts.length; depth < targetParts.length; depth += 1) if (globMatch(`/${targetParts.slice(0, depth).join("/")}`, pattern)) return true;
	return false;
}
/**
...
*/
function findDeleteDenyPatternsForLeaf(rules, target) {
	for (const rule of rules) {
...
*/
function findDeleteDenyPatterns(rules, target, hasDescendants = true) {
	const canonicalTarget = validatePath(target);
	if (!hasDescendants) return findDeleteDenyPatternsForLeaf(rules, canonicalTarget);
	const denying = [];
	const seen = /* @__PURE__ */ new Set();
	for (const rule of rules) {
		if (rule.mode !== "deny" || !rule.operations.includes("write")) continue;
		for (const pattern of rule.paths) {
			if (seen.has(pattern)) continue;
			const anchor = globAnchor(pattern);
			if (hasGlobMetaCharacter(pattern) ? wildcardDeleteOverlap(pattern, anchor, canonicalTarget) : pathsOverlap(canonicalTarget, anchor)) {
				seen.add(pattern);
...
*/
async function deleteTargetMayHaveDescendants(backend, target, permissionsConfigured) {
	if (!permissionsConfigured) return false;
	if (typeof backend.ls !== "function") return true;
	let lsResult;
	try {
	...
	if (lsResult.error) return !lsResult.error.includes("not a directory");
	if (lsResult.files && lsResult.files.length > 0) return true;
	...
	if (parentResult.error) return true;
	const targetNorm = trimTrailingSlashesFs(target);
	const matches = (parentResult.files ?? []).filter((entry) => trimTrailingSlashesFs(entry.path) === targetNorm);
	if (matches.length === 0) return true;
	return matches.some((entry) => entry.is_dir === true);
}
function trimTrailingSlashesFs(path) {
	let end = path.length;
...
`;
const DELETE_TOOL_DESCRIPTION = langchain.context`
...
function createWriteFileTool(backend, options) {
	...
	return (0, langchain.tool)(async (input, runtime) => {
		...
		schema: zod_v4.z.preprocess(normalizeFilePathInput, zod_v4.z.object({
			...
			content: zod_v4.z.string().describe("The text content to write to the file. This parameter is required.")
		}))
	});
}
/**
...
*/
function createDeleteTool(backend, options) {
	const { customDescription, permissions } = options;
	return (0, langchain.tool)(async (input, runtime) => {
		let validatedPath;
		try {
			...
			return toolError(runtime, "delete", `Error: ${getErrorMessage$1(error)}`);
		}
		const resolvedBackend = await resolveBackend(backend, runtime);
		const hasDescendants = await deleteTargetMayHaveDescendants(resolvedBackend, validatedPath, permissions.length > 0);
		const denyingPatterns = findDeleteDenyPatterns(permissions, validatedPath, hasDescendants);
		if (denyingPatterns.length > 0) return toolError(runtime, "delete", `Error: permission denied for write on ${validatedPath} (matches deny rule(s): ${denyingPatterns.join(", ")})`);
		if (!supportsDelete(resolvedBackend)) return toolError(runtime, "delete", `Error: deletion is not available for '${validatedPath}'.`);
		const result = await resolvedBackend.delete(validatedPath);
		if (result.error) return toolError(runtime, "delete", result.error);
		const message = new langchain.ToolMessage({
		...
		schema: zod_v4.z.preprocess(normalizeFilePathInput, zod_v4.z.object({ file_path: zod_v4.z.string().describe("Absolute path to the file to delete. Must be absolute, not relative.") }))
	});
}
/**
...
function allPathsScopedToRoutes(permissions, backend) {
	...
	return permissions.every((rule) => rule.paths.every((path) => prefixes.some((prefix) => {
	...
	*/
	...
	return (0, langchain.createMiddleware)({
		...
		wrapModelCall: async (request, handler) => {
			...
			if (!supportsExecution || !backendSupportsDelete) tools = tools.filter((t) => (supportsExecution || t.name !== "execute") && (backendSupportsDelete || t !== builtInDeleteTool));
...
function stripInFlightAIMessage(messages) {
...
function createTaskTool(options) {
	...
	function selectSubagent(subagentType, config) {
		...
		if (responseFormat != null && "runnable" in spec) throw new Error(`responseSchema cannot be used with compiled subagent "${spec.name}"; dynamic schemas require a declarative SubAgent spec.`);
...
function createSubAgentMiddleware(options) {
	const { defaultModel, defaultTools = [], defaultMiddleware = null, generalPurposeMiddleware = null, defaultInterruptOn = null, subagents = [], systemPrompt = null, generalPurposeAgent = true, taskDescription = null, parentSystemPrompt = null } = options;
...
function createDeepAgent(params = {}) {
	...
	let memoryMiddleware = [];
	if (memory && memory.length > 0) memoryMiddleware = [createMemoryMiddleware({
		backend,
		sources: memory,
...
function trimTrailingSlashes$1(path) {
	let end = path.length;
	while (end > 1 && path[end - 1] === "/") end--;
	return path.slice(0, end);
}
...
var StoreBackend = class {
	...
	*/
	async delete(filePath) {
		...
		const namespace = this.getNamespace();
		const items = await this.searchStorePaginated(store, namespace);
		const base = trimTrailingSlashes$1(filePath) || "/";
		const prefix = base === "/" ? "/" : `${base}/`;
		const keys = items.map((item) => String(item.key)).filter((key) => key === base || key.startsWith(prefix));
		if (keys.length === 0) return { error: `Error: File '${filePath}' not found` };
		...
		try {
			...
			return { error: `Error deleting '${filePath}': ${typeof error === "object" && error !== null && "message" in error && typeof error.message === "string" ? error.message : String(error)}` };
		}
		return {
			path: filePath,
			filesUpdate: null
...
var ContextHubBackend = class ContextHubBackend {
	...
	*/
	static collectDeleteChanges(cache, base) {
		const prefix = base === "" ? "" : `${base}/`;
		const changes = {};
		for (const key of Object.keys(cache)) if (base === "" || key === base || key.startsWith(prefix)) changes[key] = null;
		return changes;
	}
	/**
	...
	rematerializeBatch(batch, base, conflictError) {
		...
		for (const waiter of batch.waiters) {
			...
			if (intent.kind === "delete") {
				const deleteChanges = ContextHubBackend.collectDeleteChanges(cache, intent.base);
	...
	async delete(filePath) {
		...
		try {
			const accepted = await this.acceptMutation((cache) => {
				...
				const deleteChanges = ContextHubBackend.collectDeleteChanges(cache, base);
				if (Object.keys(deleteChanges).length === 0) return { result: { error: `Error: File '${filePath}' not found` } };
				return {
...
var BaseSandbox = class {
	...
	*/
	async delete(filePath) {
		...
		if (exists.exitCode !== null && exists.exitCode !== 0) return { error: `Error: '${filePath}' not found` };
