import { H as applyGrepMaxCount, J as checkEmptyContent, Q as performStringReplacement, V as SandboxError, X as isTextMimeType, Y as getMimeType, Z as normalizeReadPagination } from "./langsmith-BBV5JlNW.js";
...
var FilesystemBackend = class {
	...
	async delete(filePath) {
		try {
			...
			if (this.virtualMode && resolvedPath === this.cwd) {
				const realRoot = await fs$1.realpath(this.cwd);
				const entries = await fs$1.readdir(realRoot, { withFileTypes: true });
				await Promise.all(entries.map((entry) => fs$1.rm(path.join(realRoot, entry.name), {
					recursive: true,
					force: false
				})));
				return {
			...
			const stat = await fs$1.lstat(deletePath).catch(() => null);
			if (!stat) return { error: `Error: '${filePath}' not found` };
			if (stat.isDirectory() && !stat.isSymbolicLink()) await fs$1.rm(deletePath, {
				recursive: true,
				force: false
			});
			else await fs$1.unlink(deletePath);
			return {
				path: filePath,
				filesUpdate: null
			};
	...
	*/
	async literalSearch(pattern, baseFull, includeGlob) {
		const results = {};
		const root = (await fs$1.stat(baseFull)).isDirectory() ? baseFull : path.dirname(baseFull);
...
//# sourceMappingURL=src-CKk0apeE.js.map