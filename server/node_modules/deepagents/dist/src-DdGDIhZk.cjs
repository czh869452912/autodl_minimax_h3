...
var FilesystemBackend = class {
	...
	*/
	async read(filePath, offset = 0, limit = 500) {
		try {
			...
			const { offset: normalizedOffset, limit: normalizedLimit } = require_langsmith.normalizeReadPagination(offset, limit);
			const lines = content.split("\n");
			const totalLines = lines[lines.length - 1] === "" ? lines.length - 1 : lines.length;
			const sliceEndIdx = Math.min(normalizedOffset + normalizedLimit, lines.length);
			const endIdx = Math.min(normalizedOffset + normalizedLimit, totalLines);
			if (normalizedOffset >= totalLines) return { error: `Line offset ${normalizedOffset} exceeds file length (${totalLines} lines)` };
			const selectedLines = lines.slice(normalizedOffset, sliceEndIdx);
			if (selectedLines.length === 0 || normalizedLimit === 0) return {
			...
			return {
				...
				endLine: endIdx,
				nextOffset: endIdx < totalLines ? endIdx : void 0
			};
		} catch (e) {
	...
	async delete(filePath) {
		try {
			...
			if (this.virtualMode && resolvedPath === this.cwd) {
				const realRoot = await node_fs_promises.default.realpath(this.cwd);
				const entries = await node_fs_promises.default.readdir(realRoot, { withFileTypes: true });
				await Promise.all(entries.map((entry) => node_fs_promises.default.rm(node_path.default.join(realRoot, entry.name), {
					recursive: true,
					force: false
				})));
				return {
			...
			const stat = await node_fs_promises.default.lstat(deletePath).catch(() => null);
			if (!stat) return { error: `Error: '${filePath}' not found` };
			if (stat.isDirectory() && !stat.isSymbolicLink()) await node_fs_promises.default.rm(deletePath, {
				recursive: true,
				force: false
			});
			else await node_fs_promises.default.unlink(deletePath);
			return {
				path: filePath,
				filesUpdate: null
			};
	...
	*/
	async grep(pattern, dirPath = "/", glob = null, maxCount = null) {
		...
		return require_langsmith.applyGrepMaxCount({
			result: { matches },
			maxCount
		});
	}
	...
	*/
	async literalSearch(pattern, baseFull, includeGlob) {
		const results = {};
		const root = (await node_fs_promises.default.stat(baseFull)).isDirectory() ? baseFull : node_path.default.dirname(baseFull);
		...
		for (const fp of files) try {
			const mimeType = require_langsmith.getMimeType(fp);
			if (!require_langsmith.isTextMimeType(mimeType)) continue;
...
//# sourceMappingURL=src-DdGDIhZk.cjs.map