...
exports.getAuthorizedAudience = getAuthorizedAudience;
 ...
 */
function getAuthorizedAudience(req) {
    return req[authorizedAudience];
}
/**
 ...
 */
function isGovAuthority(authority) {
    return !!authority && /login\.microsoftonline\.us/i.test(authority);
}
function getAuthority(authConfig) {
    var _a;
    return (_a = authConfig.authorityEndpoint) !== null && _a !== void 0 ? _a : authConfig.authority;
}
/**
 ...
 */
function getWellKnownFirstPartyIssuers(authority) {
 ...
 */
function getValidIssuers(authConfig) {
    var _a;
    const authority = getAuthority(authConfig);
    const configured = authConfig.issuers && authConfig.issuers.length > 0
        ? authConfig.issuers
        : (0, settings_1.getDefaultIssuers)((_a = authConfig.tenantId) !== null && _a !== void 0 ? _a : '', authority !== null && authority !== void 0 ? authority : 'https://login.microsoftonline.com');
 ...
 */
function getEffectiveTenant(authConfig) {
    const authority = getAuthority(authConfig);
    if (authority) {
        try {
 ...
 */
function isMultiTenant(authConfig) {
    var _a;
    const tenant = (_a = getEffectiveTenant(authConfig)) === null || _a === void 0 ? void 0 : _a.toLowerCase();
 ...
 */
function isAcceptableTenantIssuer(iss, authConfig) {
 ...
 */
function validateIssuer(iss, authConfig) {
    ...
    const err = agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.JwtIssuerMismatch);
 ...
 */
function getEntraIssuerInfo(iss) {
 ...
 */
function getIssuerTenant(iss) {
    var _a;
    return (_a = getEntraIssuerInfo(iss)) === null || _a === void 0 ? void 0 : _a.tenant;
}
/**
 ...
 */
function validateTenantBinding(iss, tid) {
    ...
    if (tid.toLowerCase() !== issuerTenant) {
        const err = agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.JwtTenantMismatch);
...
function buildJwksUri(iss, authConfig) {
    ...
    switch (typeof iss === 'string' ? iss.toLowerCase() : '') {
        ...
        default:
            return `${(0, settings_1.resolveAuthority)((_a = authConfig.authorityEndpoint) !== null && _a !== void 0 ? _a : authConfig.authority, authConfig.tenantId)}/discovery/v2.0/keys`;
    }
}
...
const verifyToken = async (raw, config) => {
    ...
    const matchingEntry = config.connections && config.connections.size > 0
        ? [...config.connections.entries()].find(([_, configuration]) => typeof configuration.clientId === 'string' && audiences.includes(configuration.clientId))
        : undefined;
    if (!matchingEntry) {
