...
const logger = (0, agents_telemetry_1.debug)('agents:turnState');
...
class TurnState {
    ...
    get conversation() {
        ...
        if (!scope) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.StateNotLoaded);
        }
        return scope.value;
    }
    /**
    ...
    onComputeStorageKeys(context) {
        ...
        if (!channelId) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.MissingContextActivityChannelId);
        }
        if (!agentId) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.MissingContextActivityRecipientId);
        }
        if (!conversationId) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.MissingContextActivityConversationId);
        }
        if (!userId) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.MissingContextActivityFromId);
        }
        const keys = {};
    ...
    getScopeAndName(path) {
        ...
        if (parts.length > 2) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.InvalidStatePath, undefined, { path });
        }
        ...
        if (scope === undefined) {
            throw agents_activity_1.ExceptionHelper.generateException(Error, errorHelper_1.Errors.InvalidStateScope, undefined, { scope: parts[0] });
        }
