...
describe("errorClass", () => {
    it("never leaks the error message", () => {
        ...
        expect(["auth", "network", "timeout", "validation", "unknown"]).toContain(out);
    });
    it("categorizes by name/code", () => {
        expect(errorClass(new (class extends Error {
            name = "AbortError";
        })())).toBe("timeout");
        expect(errorClass(Object.assign(new Error("x"), { code: "ENOTFOUND" }))).toBe("network");
        expect(errorClass(new (class extends Error {
            name = "ZodError";
        })())).toBe("validation");
