...
function discoverAdapterPlatforms() {
    const found = [];
    for (const entry of readdirSync(packagesDir)) {
        if (!entry.startsWith("channels-"))
            continue;
        ...
        if (m)
            found.push({ pkg: entry, platform: m[1] });
    }
    return found;
}
describe("normalizePlatform allow-list coverage (monorepo invariant)", () => {
    it("covers every official channel adapter's declared platform", () => {
        ...
        expect(discovered.length, `expected to discover the official channel adapters under ${packagesDir}`).toBeGreaterThanOrEqual(4);
        const missing = discovered.filter(({ platform }) => normalizePlatform(platform) !== platform);
        expect(missing, `These adapter platforms are NOT in normalizePlatform's allow-list, so their ` +
            `telemetry would bucket to "custom". Add them to KNOWN_PLATFORMS in ` +
            `packages/channels/src/telemetry/sanitize-error.ts: ` +
            missing.map((m) => `"${m.platform}" (${m.pkg})`).join(", ")).toEqual([]);
    });
});
