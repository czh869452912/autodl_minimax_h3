...
test("terminal delivery tool failure stops the loop and closes renderer fanout", async () => {
    ...
    const deliveryError = new ChannelDeliveryTerminatedError("provider delivery timed out", { cause: details, details });
    let agent;
    agent = new FakeAgent([
        (subscriber) => emitBatch(subscriber, agent, "inner-run-1", [
            {
    ...
    expect(ingestedEvents.find(({ type }) => type === EventType.RUN_ERROR)).toMatchObject({
