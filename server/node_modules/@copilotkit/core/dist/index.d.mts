...
import { AbstractAgent, AgentCapabilities, AgentSubscriber, BaseEvent, Context, HttpAgent, HttpAgentConfig, Interrupt, Message, ResumeEntry, RunAgentInput, RunAgentParameters, RunAgentResult, State, Tool, ToolCall } from "@ag-ui/client";
import { DebugConfig, InspectorMetadataV1, InspectorMetadataV1 as InspectorMetadataV1$1, IntelligenceRuntimeInfo, IntelligenceRuntimeInfo as IntelligenceRuntimeInfo$1, ResolvedDebugConfig, RuntimeLicenseStatus, RuntimeLicenseStatus as RuntimeLicenseStatus$1, RuntimeMode, RuntimeMode as RuntimeMode$1, StandardSchemaV1, ThreadEndpointRuntimeInfo, ThreadEndpointRuntimeInfo as ThreadEndpointRuntimeInfo$1 } from "@copilotkit/shared";
...
declare class ProxiedCopilotRuntimeAgent extends HttpAgent {
  ...
  private ensureRuntimeConfiguration;
...
declare class AgentRegistry {
  ...
  private _inspectorMetadata?;
  private _inspectorMetadataSupported;
  private inspectorMetadataRefreshReady;
  private inspectorMetadataConnectionGeneration;
  private inspectorMetadataGeneration;
  private inspectorMetadataHeadersGeneration;
  private inspectorMetadataCredentialsGeneration;
  private inspectorMetadataAbortController?;
  private inspectorMetadataNotificationQueue;
  ...
  get inspectorMetadata(): InspectorMetadataV1$1 | undefined;
   ...
   */
  setRuntimeUrl(runtimeUrl: string | undefined, options?: {
   ...
   */
  refreshInspectorMetadata(): Promise<void>;
  private fetchInspectorMetadataRest;
  private fetchInspectorMetadataSingle;
  private isInspectorMetadataRequestCurrent;
  private invalidateInspectorMetadataConnection;
  /**
  ...
  private performRuntimeConnection;
  private fetchRuntimeInfo;
  ...
  private notifyInspectorMetadataChanged;
}
//#endregion
...
interface CopilotKitCoreContinuationHandoff {
  cancel(): void;
...
declare class StateManager {
   ...
   */
  initialize(): void;
  markNextRunAsContinuation(agent: AbstractAgent, expectedRunId?: string): CopilotKitCoreContinuationHandoff;
  /**
   ...
   */
  getRawEventForMessage(agentId: string, threadId: string, messageId: string): unknown;
  /**
 ...
 */
interface CopilotKitCoreCatalogComponent {
  name: string;
  description?: string;
  schema: unknown;
}
...
declare class RunHandler {
   ...
   */
  private _lastConnectedThreadIdsByAgent;
  private _anonymousAgentIds;
  private _nextAnonymousAgentId;
  constructor(core: CopilotKitCore);
  /**
  ...
  runAgent({
    ...
    resume,
    runId
  }: CopilotKitCoreRunAgentParams, continuationHandoff?: CopilotKitCoreContinuationHandoff): Promise<RunAgentResult>;
  /**
   * Process agent result and execute tools
   */
  private processAgentResult;
  private isFrontendPlaceholderResult;
  /**
 ...
 */
declare function createThreadSelectors(): ThreadSelectors;
interface ThreadStore {
   ...
   */
  subscribeToRunActivity?(callback: (notification: ThreadRunActivityNotification) => void): Subscription;
...
type ɵThreadSelectors = ThreadSelectors;
declare const ɵthreadAdapterEvents: ActionGroupResult<"Thread Adapter", {
  ...
  readonly newThreadStarted: EmptyActionConfig;
}>;
declare const ɵcreateThreadSelectors: typeof createThreadSelectors;
declare const ɵselectThreads: (state: ThreadState) => ThreadRecord[];
declare const ɵselectThreadsIsLoading: (state: ThreadState) => boolean;
declare const ɵselectThreadsError: (state: ThreadState) => Error | null;
declare const ɵselectFetchMoreError: (state: ThreadState) => Error | null;
declare const ɵselectHasNextPage: (state: ThreadState) => boolean;
declare const ɵselectIsFetchingNextPage: (state: ThreadState) => boolean;
declare const ɵselectIsMutating: (state: ThreadState) => boolean;
/**
...
type MemoryErrorCategory = "validation" | "auth" | "permission" | "not_found" | "conflict" | "rate_limit" | "internal" | "dependency";
/** Stable codes for the memory store's surfaced errors. */
type MemoryErrorCode = "MEMORY_LIST_FAILED" | "MEMORY_RECALL_FAILED" | "MEMORY_CREDENTIALS_FAILED" | "MEMORY_MUTATION_FAILED" | "MEMORY_REQUEST_TIMEOUT";
/** A single registry entry describing one memory error code. */
interface MemoryErrorRegistryEntry {
 ...
 *   slow).
 */
declare const MEMORY_ERROR_REGISTRY: Readonly<Record<MemoryErrorCode, MemoryErrorRegistryEntry>>;
/**
...
declare class MemoryError extends Error {
   ...
   */
  constructor(code: MemoryErrorCode, options?: {
    message?: string;
    retryable?: boolean;
    cause?: unknown;
  });
 ...
 *   signal flips.
 */
type MemoryRealtimeStatus = "connecting" | "connected" | "unavailable";
/**
...
declare const memoryDomainEvents: ActionGroupResult<"Memory Domain", {
  readonly memoryUpserted: PropsActionConfig<{
    sessionId: number;
    memory: Memory;
  }>;
  readonly memoryInvalidated: PropsActionConfig<{
    sessionId: number;
    memoryId: string;
  }>;
  readonly realtimeConnecting: PropsActionConfig<{
    sessionId: number;
  }>;
  readonly realtimeConnected: PropsActionConfig<{
    sessionId: number;
  }>;
  readonly realtimeUnavailable: PropsActionConfig<{
    sessionId: number;
  }>;
}>;
 ...
 */
declare function mapMemoryMetadataEvent(event: MemoryMetadataEvent, sessionId: number): ReturnType<typeof memoryDomainEvents.memoryUpserted> | ReturnType<typeof memoryDomainEvents.memoryInvalidated>;
/**
 ...
 */
declare function createMemoryStore(environment: MemoryEnvironment): MemoryStore;
type ɵMemoryState = MemoryState;
type ɵMemoryRuntimeContext = MemoryRuntimeContext;
type ɵMemoryMetadataEvent = MemoryMetadataEvent;
type ɵMemoryEnvironment = MemoryEnvironment;
type ɵMemoryStore = MemoryStore;
declare const ɵmemoryAdapterEvents: ActionGroupResult<"Memory Adapter", {
  readonly started: EmptyActionConfig;
  readonly stopped: EmptyActionConfig;
  readonly contextChanged: PropsActionConfig<{
    context: MemoryRuntimeContext | null;
  }>;
  readonly addRequested: PropsActionConfig<{
    requestId: string;
    input: NewMemory;
  }>;
  readonly updateRequested: PropsActionConfig<{
    requestId: string;
    id: string;
    changes: MemoryChanges;
  }>;
  readonly removeRequested: PropsActionConfig<{
    requestId: string;
    id: string;
  }>;
}>;
declare const ɵmemoryRestEvents: ActionGroupResult<"Memory REST", {
  readonly listRequested: PropsActionConfig<{
    sessionId: number;
  }>;
  readonly listSucceeded: PropsActionConfig<{
    sessionId: number;
    memories: Memory[];
  }>;
  readonly listFailed: PropsActionConfig<{
    sessionId: number;
    error: Error;
  }>;
  readonly listUnavailable: PropsActionConfig<{
    sessionId: number;
  }>;
  readonly mutationFinished: PropsActionConfig<{
    outcome: MemoryMutationOutcome;
  }>;
  readonly credentialsRequested: PropsActionConfig<{
    sessionId: number;
  }>;
  readonly credentialsSucceeded: PropsActionConfig<{
    sessionId: number;
    joinToken?: string;
    joinCode?: string;
  ...
  readonly credentialsFailed: PropsActionConfig<{
    sessionId: number;
    error: Error;
  }>;
  readonly credentialsUnavailable: PropsActionConfig<{
    sessionId: number;
  }>;
}>;
declare const ɵmemoryDomainEvents: ActionGroupResult<"Memory Domain", {
  readonly memoryUpserted: PropsActionConfig<{
    sessionId: number;
    memory: Memory;
  }>;
...
declare const ɵmemoryReducer: Reducer<MemoryState>;
declare const ɵmapMemoryMetadataEvent: typeof mapMemoryMetadataEvent;
declare const ɵselectMemories: Selector<MemoryState, Memory[]>;
declare const ɵselectMemoriesIsLoading: Selector<MemoryState, boolean>;
declare const ɵselectMemoriesError: Selector<MemoryState, Error | null>;
declare const ɵselectMemoriesIsMutating: Selector<MemoryState, boolean>;
declare const ɵselectMemoriesAvailable: Selector<MemoryState, boolean>;
declare const ɵselectMemoriesRealtimeStatus: Selector<MemoryState, MemoryRealtimeStatus>;
//#endregion
...
declare class IntelligenceAgent extends AbstractAgent {
  ...
  private advanceLastSeenEventId;
  private readEventId;
  private readControlEventId;
  private readDurableEventId;
 ...
 */
declare const ɵCOPILOTKIT_FEATURES: readonly ["chat", "sidebar", "popup", "threads"];
/**
 * Union of recognised CopilotKit feature names.
 */
type ɵCopilotKitFeature = (typeof ɵCOPILOTKIT_FEATURES)[number];
/**
 ...
 * ```
 */
declare function ɵisCopilotKitFeature(name: string): name is ɵCopilotKitFeature;
//#endregion
//#region src/interrupt-state.d.ts
/** @internal Application authors must not depend on this contract. */
interface ɵInterruptEvent<TValue = unknown> {
  name: string;
  value: TValue;
}
/** @internal Application authors must not depend on this contract. */
type ɵPendingInterrupt<TValue = unknown> = {
  kind: "legacy";
...
/** @internal Application authors must not depend on this contract. */
interface ɵInterruptToolResult {
  toolCallId: string;
  content: string;
}
/** @internal Application authors must not depend on this contract. */
 ...
 */
declare class ɵInterruptState<TValue = unknown> {
  #private;
...
//#endregion
//#region src/inspector-thread-bridge.d.ts
declare const INSPECTOR_THREAD_BRIDGE_PLUGIN_ID: "cpk-inspector-thread";
type InspectorViewThreadPayload = {
  requestId: string;
  threadId: string;
  agentId: string;
};
type InspectorStopViewingPayload = {
  requestId: string;
  agentId: string;
};
type InspectorActiveThreadPayload = {
  requestId: string;
  threadId: string;
  agentId: string;
  source: "app" | "override";
};
type InspectorViewThreadResultPayload = {
  requestId: string;
  threadId: string;
  agentId: string;
  ok: true;
...
declare function isInspectorThreadBridgeEnabled(): boolean;
declare function createInspectorThreadRequestId(): string;
/**
 ...
 */
declare function emitInspectorViewThread(payload: InspectorViewThreadPayload): boolean;
declare function emitInspectorStopViewing(payload: InspectorStopViewingPayload): void;
declare function emitInspectorActiveThread(payload: InspectorActiveThreadPayload): void;
declare function emitInspectorViewThreadResult(payload: InspectorViewThreadResultPayload): void;
declare function onInspectorViewThread(handler: (payload: InspectorViewThreadPayload) => boolean): () => void;
declare function onInspectorStopViewing(handler: (payload: InspectorStopViewingPayload) => void): () => void;
declare function onInspectorActiveThread(handler: (payload: InspectorActiveThreadPayload) => void): () => void;
declare function onInspectorViewThreadResult(handler: (payload: InspectorViewThreadResultPayload) => void): () => void;
//#endregion
export { ActionConfig, ActionCreator, ActionFromCreator, ActionFromCreators, ActionGroupConfig, ActionGroupResult, AgentRegistry, AgentThreadLockedError, AnyAction, ContextStore, CopilotKitCore, type CopilotKitCoreAddAgentParams, type CopilotKitCoreCatalogComponent, CopilotKitCoreConfig, type CopilotKitCoreConnectAgentParams, CopilotKitCoreContinuationHandoff, CopilotKitCoreErrorCode, CopilotKitCoreFriendsAccess, CopilotKitCoreGetSuggestionsResult, type CopilotKitCoreGetToolParams, type CopilotKitCoreRegisterProxiedAgentParams, type CopilotKitCoreRegisterProxiedAgentResult, type CopilotKitCoreRunAgentParams, type CopilotKitCoreRunToolParams, type CopilotKitCoreRunToolResult, CopilotKitCoreRuntimeConnectionStatus, CopilotKitCoreStopAgentParams, CopilotKitCoreSubscriber, CopilotKitCoreSubscription, CopilotRuntimeTransport, DispatchingEffect, DispatchingEffectOptions, DynamicSuggestionsConfig, Effect, EmptyActionConfig, FrontendTool, FrontendToolHandlerContext, INSPECTOR_THREAD_BRIDGE_PLUGIN_ID, InspectorActiveThreadPayload, type InspectorMetadataV1, InspectorStopViewingPayload, InspectorViewThreadPayload, InspectorViewThreadResultPayload, IntelligenceAgent, IntelligenceAgentConfig, type IntelligenceRuntimeInfo, MEMORY_ERROR_REGISTRY, Memory, MemoryChanges, MemoryError, type MemoryErrorCategory, type MemoryErrorCode, type MemoryErrorRegistryEntry, MemoryKind, MemoryRealtimeStatus, MemoryScope, NewMemory, NonDispatchingEffect, NonDispatchingEffectOptions, PropsActionConfig, ProxiedCopilotRuntimeAgent, ProxiedCopilotRuntimeAgentConfig, Reducer, RunCompletionAware, RunHandler, type RuntimeLicenseStatus, type RuntimeMode, ScopedContext, Selector, StateManager, StaticSuggestionsConfig, Store, StoreLifecycleAction, SubscribeToAgentOptions, SubscribeToAgentSubscriber, Suggestion, SuggestionAvailability, SuggestionEngine, SuggestionsConfig, type ThreadEndpointRuntimeInfo, ThreadRunActivityNotification, ToolCallStatus, completePartialMarkdown, createActionGroup, createEffect, createInspectorThreadRequestId, createReducer, createSelector, createStore, emitInspectorActiveThread, emitInspectorStopViewing, emitInspectorViewThread, emitInspectorViewThreadResult, empty, ensureObjectArgs, isAbortError, isInspectorThreadBridgeEnabled, isRunCompletionAware, ofType, on, onInspectorActiveThread, onInspectorStopViewing, onInspectorViewThread, onInspectorViewThreadResult, parseToolArguments, props, select, ɵCOPILOTKIT_FEATURES, ɵCopilotKitFeature, ɵInterruptDecision, ɵInterruptEvent, ɵInterruptState, ɵInterruptToolResult, ɵMAX_SOCKET_RETRIES, ɵMemoryEnvironment, ɵMemoryMetadataEvent, ɵMemoryRuntimeContext, ɵMemoryState, ɵMemoryStore, ɵPendingInterrupt, ɵPhoenixChannelLike, ɵPhoenixChannelOptions, ɵPhoenixChannelSession, ɵPhoenixJoinOutcome, ɵPhoenixPushLike, ɵPhoenixSocketLike, ɵPhoenixSocketOptions, ɵPhoenixSocketSession, ɵPhoenixSocketSignal, ɵThread, ɵThreadEnvironment, ɵThreadMetadataEvent, ɵThreadRuntimeContext, ɵThreadSelectors, ɵThreadStore, createMemoryStore as ɵcreateMemoryStore, ɵcreateThreadSelectors, createThreadStore as ɵcreateThreadStore, ɵisCopilotKitFeature, isRetryableStatus as ɵisRetryableMemoryStatus, ɵjoinPhoenixChannel$, ɵmapMemoryMetadataEvent, ɵmemoryAdapterEvents, ɵmemoryDomainEvents, ɵmemoryReducer, ɵmemoryRestEvents, ɵobservePhoenixEvent$, ɵobservePhoenixJoinOutcome$, ɵobservePhoenixSocketHealth$, ɵobservePhoenixSocketSignals$, ɵphoenixChannel$, ɵphoenixSocket$, ɵselectFetchMoreError, ɵselectHasNextPage, ɵselectIsFetchingNextPage, ɵselectIsMutating, ɵselectMemories, ɵselectMemoriesAvailable, ɵselectMemoriesError, ɵselectMemoriesIsLoading, ɵselectMemoriesIsMutating, ɵselectMemoriesRealtimeStatus, ɵselectThreads, ɵselectThreadsError, ɵselectThreadsIsLoading, ɵthreadAdapterEvents };
