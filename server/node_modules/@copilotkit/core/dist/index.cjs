...
function createMemoryStore(environment) {
	...
	const store = createStore({
		...
		effects: [
			...
			createEffect((actions$, state$) => actions$.pipe(ofType(memoryAdapterEvents.addRequested, memoryAdapterEvents.updateRequested, memoryAdapterEvents.removeRequested), (0, rxjs_operators.withLatestFrom)(state$), (0, rxjs_operators.mergeMap)(([action, state]) => {
				...
				if (!context?.runtimeUrl) return (0, rxjs.of)(memoryRestEvents.mutationFinished({ outcome: {
	...
	return {
		...
		recall(query, opts) {
			...
			}), (0, rxjs_operators.map)((data) => (data.memories ?? []).map((m) => recallResponseToMemory(m)))));
		},
		addMemory(input) {
...
function createThreadStore(environment) {
	...
	const store = createStore({
		...
		effects: [
			...
			createEffect((actions$, state$) => actions$.pipe(ofType(threadSocketEvents.metadataReceived), (0, rxjs_operators.withLatestFrom)(state$), (0, rxjs_operators.filter)(([action, state]) => action.sessionId === state.sessionId && threadMetadataTargetsAgent(action.payload, state.context?.agentId)), (0, rxjs_operators.map)(([action, state]) => {
...
var ɵInterruptState = class {
	...
	#respond(interrupts, interruptId, response) {
		...
		const resume = (0, _ag_ui_client.buildResumeArray)(mutableInterrupts, this.#responses);
