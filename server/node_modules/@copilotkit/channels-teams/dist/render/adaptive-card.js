...
export function renderAdaptiveCard(ir) {
    ...
    for (const node of ir)
        renderNode(node, body, actions, context);
    const card = {
        type: "AdaptiveCard",
        $schema: SCHEMA,
        version: VERSION,
...
function renderNode(node, body, actions, context) {
    ...
    switch (node.type) {
        ...
        case "button":
            ...
            return;
        case "select":
            body.push(renderSelect(node, fieldId(node, "onSelect", "select", context)));
            return;
        case "input":
...
function fieldId(node, handlerProp, fallback, context) {
    const props = node.props ?? {};
    const index = ++context.nextFieldIndex;
    const rawName = typeof props.name === "string" ? props.name.trim() : "";
    const explicitName = rawName && rawName !== "ckActionId" && rawName !== "value"
        ? rawName
        : undefined;
