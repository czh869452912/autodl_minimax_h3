...
const manifest = new Map(TEAMS_NATIVE_MANIFEST.map((entry) => [
    `${entry.kind}:${entry.component}`,
    entry,
]));
/** Validate and serialize one explicit native Adaptive Card root. */
export function renderTeamsNativeCard(ir) {
...
export function containsTeamsNative(ir) {
    return ir.some((node) => isNativeNode(node));
}
function serializeTeamsNode(node, path) {
    ...
    const entry = manifest.get(`${node.props.nativeKind}:${node.props.nativeType}`);
    if (!entry) {
    ...
    if (children !== undefined) {
        ...
        if (node.props.nativeKind === "root") {
            ...
            for (const [index, child] of body.entries()) {
                if (!isRootBody(child)) {
                    throw new Error(`${path}.body[${index}]: ${child.props.nativeType} is not allowed in an AdaptiveCard body.`);
                }
            }
            output.body = body.map((child, index) => serializeTeamsNode(child, `${path}.body[${index}]`));
            if (actions.length > 0) {
                output.actions = actions.map((child, index) => serializeTeamsNode(child, `${path}.actions[${index}]`));
            }
        }
        else {
            output[entry.childrenSlot] = nodes.map((child, index) => serializeTeamsNode(child, `${path}.${entry.childrenSlot}[${index}]`));
        }
    }
...
function serializeValue(value, path) {
    if (isNativeNode(value))
        return serializeTeamsNode(value, path);
    if (Array.isArray(value)) {
        return value.map((item, index) => serializeValue(item, `${path}[${index}]`));
    }
    return value;
}
function asNativeNodes(value, path) {
...
function requiredVersion(root) {
    return highestVersionNode(root).version;
}
function highestVersionNode(root) {
...
function visitNative(node, visitor) {
    visitor(node);
    for (const value of Object.values(node.props)) {
        if (isNativeNode(value))
            visitNative(value, visitor);
        if (Array.isArray(value)) {
            for (const item of value)
                if (isNativeNode(item))
                    visitNative(item, visitor);
        }
    }
}
...
function assertRawIsNonInteractive(value, path) {
