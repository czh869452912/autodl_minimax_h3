...
import { ChannelProviderMismatchError, ChannelProviderDeliveryError, ClaimedChannelDelivery, ChannelDeliveryTransport, safeChannelErrorMetadata, } from "./delivery-transport.js";
...
function claimResult(deliveryId, ownerGeneration = 7) {
    return {
        result: "claimed",
        deliveryId,
        ownerGeneration,
...
function invitation(deliveryId, canonicalThreadId, adapter = "slack") {
    return {
        protocol: "channel_delivery_v1",
        deliveryId,
        canonicalThreadId,
...
function acknowledgement(packet, fields = {}) {
...
function channel(joinReply = preparedDelivery()) {
    return {
        joinReply,
        push: vi
            .fn()
            .mockImplementation((_event, packet) => Promise.resolve(acknowledgement(packet))),
        on: vi.fn(),
        onClose: vi.fn(),
        leave: vi.fn(),
    };
...
async function runTranscriptFailure(input) {
    ...
    const appApiFetch = vi.fn(async (_url) => Promise.resolve(new Response(JSON.stringify({
        error: {
    ...
    invitationHandler(invitation(delivery.deliveryId, delivery.canonicalThreadId, delivery.adapter));
    await vi.waitFor(() => expect(deliveryChannel.leave).toHaveBeenCalledOnce());
    await transport.stop();
    return {
        appApiFetch,
        packets: vi.mocked(deliveryChannel.push).mock.calls.map(([, packet]) => packet.payload),
    };
}
async function runHandlerFailure(delivery, handler) {
    const deliveryChannel = channel(delivery);
    const control = {
        push: vi.fn().mockResolvedValue(claimResult(delivery.deliveryId)),
        on: vi.fn(),
    ...
    return vi
        ...
        .mock.calls.map(([, packet]) => packet.payload);
}
test("transcript failure posts the generic unmetered error for an app mention", async () => {
    ...
    expect(result.appApiFetch.mock.calls.every(([url]) => String(url).endsWith("/transcript"))).toBe(true);
...
test.each([
...
])("%s welcome failure posts the fixed provider error before failing the delivery", async (adapter, effectKind) => {
    const base = preparedDelivery();
    const delivery = {
        ...base,
        adapter,
    ...
    expect(vi
        .mocked(deliveryChannel.push)
        .mock.calls.map(([, packet]) => packet.payload)).toEqual([
        {
            kind: effectKind,
            text: "Something went wrong",
        },
...
test("ambient handler failure is silent until developer output is expected", async () => {
    ...
    await expect(runHandlerFailure(delivery, async () => {
    ...
    await expect(runHandlerFailure(delivery, async (claimed) => {
...
test("rejects a wrong-provider effect before charging or sending a packet", async () => {
    ...
    const claimed = new ClaimedChannelDelivery(preparedDelivery(), { ownerGeneration: 1, runtimeInstanceId: "rti_runtime_01" }, deliveryChannel, vi.fn(), undefined, undefined, new AbortController().signal, { charge });
    await expect(claimed.effect("response_wrong_provider", {
        kind: "teams.message.create",
...
test("rejects a raw provider message id at the Gateway acknowledgement boundary", async () => {
    const deliveryChannel = channel();
    vi.mocked(deliveryChannel.push).mockImplementationOnce((_event, packet) => Promise.resolve(acknowledgement(packet, {
        result: {
            providerReference: "pref_v1_message_01",
            providerMessageId: "1712345678.1234",
        },
    })));
    const claimed = new ClaimedChannelDelivery(preparedDelivery(), { ownerGeneration: 1, runtimeInstanceId: "rti_runtime_01" }, deliveryChannel, vi.fn());
    await expect(claimed.effect("response_raw_provider_id", {
        kind: "slack.message.create",
        text: "Hello",
...
test("rejects acknowledgement fields that drift from the strict Gateway schema", async () => {
    ...
    for (const fields of malformed) {
        const deliveryChannel = channel();
        vi.mocked(deliveryChannel.push).mockImplementationOnce((_event, packet) => Promise.resolve(acknowledgement(packet, fields)));
        const claimed = new ClaimedChannelDelivery(preparedDelivery(), { ownerGeneration: 1, runtimeInstanceId: "rti_runtime_01" }, deliveryChannel, vi.fn());
        await expect(claimed.effect("response_malformed_ack", {
            kind: "slack.message.create",
            text: "Hello",
...
test("wrong-provider handler output uses only the trusted active-provider fallback", async () => {
    const base = preparedDelivery();
    await expect(runHandlerFailure({ ...base, surfaceKind: "direct_message" }, async (claimed) => {
...
test("adapts an in-flight Slack append across new-old-new gateway rejoins", async () => {
    ...
    const initialPacket = vi.mocked(first.push).mock.calls[0][1];
    const retriedPacket = vi.mocked(second.push).mock.calls[0][1];
    const restoredPacket = vi.mocked(third.push).mock.calls[0][1];
...
test("claims bounded pending work but does not execute above the local limit", async () => {
    ...
    const control = {
        ...
        join: vi.fn().mockImplementation((topic) => {
            ...
            return Promise.resolve(channel({
                ...preparedDelivery(),
                deliveryId,
                canonicalThreadId: deliveryId === "dlv_delivery_01" ? "thread_01" : "thread_02",
            }));
        }),
    };
    const transport = new ChannelDeliveryTransport({
    ...
    const handled = vi.fn(async (_session, delivery) => {
    ...
    expect(control.push).toHaveBeenCalledWith("claim", expect.objectContaining({ deliveryId: "dlv_delivery_02" }));
...
test("pending overflow records an explicit outcome without growing the buffer", async () => {
    ...
    releaseFirst?.();
    await vi.waitFor(() => expect(handled).toHaveBeenCalledTimes(2));
    expect(handled).not.toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ deliveryId: "dlv_delivery_03" }));
    await transport.stop();
});
test("claims same-Thread work for Redis coordination but executes it in order", async () => {
    ...
    await vi.waitFor(() => expect(handled).toHaveBeenCalledTimes(2));
    expect(control.push).toHaveBeenCalledWith("claim", expect.objectContaining({ deliveryId: "dlv_delivery_02" }));
    expect(control.push).toHaveBeenCalledWith("claim", expect.objectContaining({ deliveryId: "dlv_delivery_03" }));
    releaseFirst?.();
...
test("a newer same-Thread claim aborts the exact switchable delivery before output", async () => {
    ...
    const supersessionHandlers = new Map();
    const handled = [];
    ...
    await vi.waitFor(() => expect(handled).toEqual(["dlv_delivery_01", "dlv_delivery_02"]));
...
test("later same-Thread work waits FIFO when Redis reports a committed owner", async () => {
    ...
    await vi.waitFor(() => expect(handled).toEqual(["dlv_delivery_01"]));
    invite(invitation("dlv_delivery_02", "thread_01"));
    await vi.waitFor(() => expect(control.push).toHaveBeenCalledWith("claim", expect.objectContaining({ deliveryId: "dlv_delivery_02" })));
...
test("retries the exact packet after reconnect and calls no second sequence", async () => {
    ...
    const session = new ClaimedChannelDelivery(preparedDelivery(), {
        ownerGeneration: 7,
        runtimeInstanceId: "rti_runtime_01",
    }, first, reconnect);
    ...
    const retried = vi.mocked(second.push).mock.calls[0][1];
    const original = vi.mocked(first.push).mock.calls[0][1];
...
test("polls the same packet after a retry-wait result", async () => {
    ...
    vi.mocked(deliveryChannel.push)
        .mockImplementationOnce((_event, packet) => Promise.resolve(acknowledgement(packet, {
        phase: "retry_wait",
...
test("keeps a confirmed Teams image capability rejection non-terminal", async () => {
    ...
    const claimedDelivery = new ClaimedChannelDelivery({ ...preparedDelivery(), adapter: "teams" }, {
        ownerGeneration: 7,
        runtimeInstanceId: "rti_runtime_01",
    }, deliveryChannel, vi.fn());
    await expect(claimedDelivery.effect("response_image", {
    ...
    await expect(claimedDelivery.effect("response_text", {
        kind: "teams.message.create",
        text: "Text fallback",
    })).resolves.toEqual({
    ...
    vi.mocked(deliveryChannel.push).mockImplementation((_event, packet) => {
        ...
        if (body.payload?.kind === "channel.delivery.terminal") {
            ...
            if (terminalAttempts === 1) {
                ...
                return Promise.reject(new RealtimeGatewayPushError("packet", "conflict", "terminal push dropped"));
            }
        }
...
test("refreshes owner generation on packets after reconnect", async () => {
    ...
    const secondPacket = vi.mocked(second.push).mock.calls[1][1];
...
test("closes the packet path after a permanent push failure", async () => {
    const deliveryChannel = channel();
    const { RealtimeGatewayPushError } = await import("./realtime-gateway.js");
    vi.mocked(deliveryChannel.push).mockRejectedValue(new RealtimeGatewayPushError("packet", "conflict", "sequence conflict"));
    const session = new ClaimedChannelDelivery(preparedDelivery(), {
        ownerGeneration: 7,
        runtimeInstanceId: "rti_runtime_01",
    }, deliveryChannel, vi.fn());
    await expect(session.effect("response_01", {
        kind: "slack.message.create",
        text: "Hello",
    })).rejects.toBeInstanceOf(RealtimeGatewayPushError);
    await expect(session.effect("response_02", {
        kind: "slack.message.create",
        text: "World",
...
test("still allows stream.stop after a permanent non-terminal failure", async () => {
    ...
    vi.mocked(deliveryChannel.push).mockImplementation((_event, packet) => {
        ...
        if (body.payload?.kind === "slack.stream.append") {
            return Promise.reject(new RealtimeGatewayPushError("packet", "conflict", "append failed"));
        }
    ...
    const kinds = vi
        .mocked(deliveryChannel.push)
        .mock.calls.map((call) => call[1].payload?.kind);
...
test("classifies timeout/expiry errors from message text", async () => {
    expect(safeChannelErrorMetadata(new Error("realtime gateway delivery join timed out"))).toEqual({ errorCategory: "timeout" });
    expect(safeChannelErrorMetadata(new Error("Channel delivery ownership expired"))).toEqual({ errorCategory: "timeout" });
});
async function expectPreparedInputRejected(input) {
...
async function expectPreparedDeliveryRejected(badPrepared) {
    const deliveryChannel = channel(badPrepared);
    const control = {
    ...
    const terminalPackets = vi
        .mocked(deliveryChannel.push)
        .mock.calls.filter((call) => call[1].payload?.kind ===
...
test("accepts a namespaced Teams app user id longer than one external id", async () => {
    ...
    const invitationHandler = vi.mocked(control.on).mock.calls[0][1];
    invitationHandler(invitation(delivery.deliveryId, delivery.canonicalThreadId, "teams"));
...
test("surfaces a failed provider result as an already-terminal error", async () => {
    ...
    const deliveryChannel = channel();
    vi.mocked(deliveryChannel.push).mockImplementation((_event, packet) => Promise.resolve(acknowledgement(packet, {
        phase: "failed",
        result: {
