...
test("interaction update and delete preserve the opaque provider reference", async () => {
    ...
    channel.onInteraction("approve", async ({ thread, message }) => {
        await thread.update(message.ref, Section({ children: "Updated interaction message" }));
    ...
    try {
        await gateway.deliver(preparedDelivery("interaction_reference", "slack", {
            kind: "interaction",
            actionId: "approve",
...
test("invalid interaction references fail before handler dispatch", async () => {
    ...
    try {
        await gateway.deliver(preparedDelivery("invalid_reference", "slack", {
            kind: "interaction",
            actionId: "approve",
