...
import { ChannelFileDeliveryUnknownError, DeliveryAdapter, } from "./delivery-adapter.js";
...
function replyTarget(session, adapter = "slack") {
    return {
...
describe("DeliveryAdapter.postFile", () => {
    it("soft-returns upload failures without throwing", async () => {
        ...
        await expect(adapter.postFile(replyTarget(session), {
    ...
    it("rethrows permanent effect failures so claimAndHandle cannot complete", async () => {
        const session = {
            ...
            effect: vi
                .fn()
                .mockRejectedValue(new RealtimeGatewayPushError("packet", "conflict", "file create failed")),
        };
        ...
        await expect(adapter.postFile(replyTarget({
            uploadFile: vi.fn().mockResolvedValue("file_handle_01"),
            effect: vi
                .fn()
                .mockRejectedValue(new ChannelProviderDeliveryError("provider_failed", "failed")),
        }), { bytes: new Uint8Array([1]), filename: "a.png" })).rejects.toBeInstanceOf(ChannelProviderDeliveryError);
        await expect(adapter.postFile(replyTarget({
            uploadFile: vi.fn().mockResolvedValue("file_handle_01"),
            effect: vi
                .fn()
                .mockRejectedValue(new TypeError("Gateway returned a conflicting packet acknowledgement")),
        }), { bytes: new Uint8Array([1]), filename: "a.png" })).rejects.toBeInstanceOf(TypeError);
    });
    it("returns ok after a successful file create effect", async () => {
        const onEvent = vi.fn();
        const runCanonical = vi.fn(async (args) => {
            await args.execute({ onEvent }, { threadId: args.threadId, runId: args.runId });
        ...
        const agentSession = await adapter.conversationStore.getOrCreate("thread_postfile", target, () => new NoopAgent());
        await expect(adapter.postFile(target, {
            bytes: new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]),
            filename: "a.png",
            title: "chart",
            altText: "Line chart",
        })).resolves.toEqual({ ok: true, assetId: "file_handle_01" });
        expect(session.effect).toHaveBeenCalledWith(expect.stringMatching(/^response_/), expect.objectContaining({
    ...
    it("throws a typed unknown error after exhausted ambiguous delivery", async () => {
        const session = {
            ...
            effect: vi
                .fn()
                .mockRejectedValue(new ChannelProviderDeliveryError("file_delivery_unknown", "uncertain")),
        };
        const adapter = makeAdapter();
    ...
    it("keeps confirmed success when canonical history retries exhaust", async () => {
        ...
        const target = replyTarget(session);
        await adapter.conversationStore.getOrCreate("thread_postfile", target, () => new NoopAgent());
        await expect(adapter.postFile(target, {
            bytes: new Uint8Array([1]),
            filename: "a.txt",
        })).resolves.toEqual({ ok: true, assetId: "file_handle_01" });
        await vi.waitFor(() => {
            expect(log).toHaveBeenCalledWith("channel managed asset history", expect.objectContaining({
                outcome: "failed",
    ...
    it("returns a Teams capability error and keeps later text usable", async () => {
        ...
        await expect(adapter.post(target, [
            {
                type: "text",
        ...
        expect(session.effect).toHaveBeenNthCalledWith(2, expect.stringMatching(/^response_/), {
    ...
    it("delivers a general Teams file through the managed file effect", async () => {
        ...
        await expect(makeAdapter().postFile(replyTarget(session, "teams"), {
            bytes: new TextEncoder().encode("report"),
            filename: "report.txt",
            title: "Weekly report",
        })).resolves.toEqual({ ok: true, assetId: "file_handle_01" });
        expect(session.effect).toHaveBeenCalledWith(expect.stringMatching(/^response_/), {
