...
export const CHANNEL_DELIVERY_PROTOCOL = "channel_delivery_v1";
/** Negotiates authoritative snapshots for retry-safe Slack stream appends. */
export const SLACK_STREAM_APPEND_FULL_TEXT_CAPABILITY = "slack_stream_append_full_text_v1";
...
export function assertProviderReference(value) {
...
export function assertProviderMessageId(value) {
...
export function isProviderMessageId(value) {
    try {
...
export function isValidReactionName(value) {
...
export function deliveryPacketByteLength(value) {
    return new TextEncoder().encode(JSON.stringify(value)).byteLength;
}
/** Validate one exact, bounded, destination-free delivery packet. */
export function assertDeliveryPacket(value) {
    if (!isRecord(value)) {
...
function isDeliveryPayload(value) {
    if (!isRecord(value) || typeof value.kind !== "string")
        return false;
    switch (value.kind) {
        case "slack.stream.start":
            return (hasExactFields(value, ["kind", "initialText"], ["initialText"]) &&
                optionalBoundedString(value.initialText, 40_000));
        case "slack.stream.append":
            return (hasExactFields(value, ["kind", "providerReference", "delta", "fullText"], ["fullText"]) &&
        ...
        case "slack.stream.task":
            return (hasExactFields(value, [
                ...
                ["pending", "in_progress", "complete", "failed"].includes(String(value.status)));
        case "slack.stream.stop":
            return (hasExactFields(value, ["kind", "providerReference"]) &&
                validReference(value.providerReference));
        case "slack.thread.status":
            return (hasExactFields(value, ["kind", "status", "loadingMessages"], ["loadingMessages"]) &&
                boundedString(value.status, 0, 512) &&
                optionalBoundedStringArray(value.loadingMessages, 10, 512));
        case "slack.message.create":
            return (hasExactFields(value, ["kind", "text", "blocks"], ["blocks"]) &&
                boundedString(value.text, 0, 40_000) &&
                optionalRecordArray(value.blocks, 100));
        case "slack.message.replace":
            return (hasExactFields(value, ["kind", "providerReference", "text", "blocks"], ["blocks"]) &&
                validReference(value.providerReference) &&
                boundedString(value.text, 0, 40_000) &&
                optionalRecordArray(value.blocks, 100));
        ...
        case "teams.message.create":
            return (hasExactFields(value, ["kind", "text", "cards"], ["cards"]) &&
                boundedString(value.text, 0, 40_000) &&
                optionalRecordArray(value.cards, 25));
        case "teams.message.replace":
        case "teams.message.finalize":
            return (hasExactFields(value, ["kind", "providerReference", "text", "cards"], ["cards"]) &&
                validReference(value.providerReference) &&
                boundedString(value.text, 0, 40_000) &&
                optionalRecordArray(value.cards, 25));
        ...
        case "teams.reaction.remove":
            return (hasExactFields(value, ["kind", "providerReference", "reaction"]) &&
                validReference(value.providerReference) &&
                isValidReactionName(value.reaction));
        case "slack.file.create":
            return (hasExactFields(value, ["kind", "fileHandle", "title", "altText"], ["title", "altText"]) &&
        ...
        case "teams.file.create":
            return (hasExactFields(value, ["kind", "fileHandle", "filename", "title", "altText"], ["title", "altText"]) &&
                boundedString(value.fileHandle, 1, 128) &&
                boundedString(value.filename, 1, 512) &&
                optionalBoundedString(value.title, 512) &&
                optionalBoundedString(value.altText, 2_000));
        case "teams.file.consent.complete":
            return (hasExactFields(value, ["kind", "fileHandle"]) &&
                boundedString(value.fileHandle, 1, 128));
        case "slack.image.create":
        case "teams.image.create":
            return (hasExactFields(value, ["kind", "fileHandle", "altText"]) &&
                boundedString(value.fileHandle, 1, 128) &&
                boundedString(value.altText, 1, 2_000));
        case "channel.delivery.terminal":
            return (hasExactFields(value, ["kind", "status", "code"]) &&
                ["complete", "failed_before_output", "failed", "uncertain"].includes(String(value.status)) &&
                [
...
function optionalBoundedStringArray(value, maxItems, maxLength) {
    return (value === undefined ||
        (Array.isArray(value) &&
            value.length <= maxItems &&
            value.every((item) => boundedString(item, 1, maxLength))));
}
function optionalRecordArray(value, max) {
    return (value === undefined ||
        (Array.isArray(value) &&
            value.length <= max &&
