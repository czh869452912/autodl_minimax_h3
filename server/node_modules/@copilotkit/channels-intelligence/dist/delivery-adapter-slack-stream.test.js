...
import { ChannelDeliveryTransport, ClaimedChannelDelivery, } from "./delivery-transport.js";
...
function preparedDelivery(capabilities = [
...
function setup(capabilities, options = {}) {
    ...
    const deliveryChannel = {
        ...
        push: vi.fn(async (_event, value) => {
            ...
            if (value.payload.kind === "channel.delivery.terminal" ||
            ...
            if (options.dropStreamStartCapability &&
                value.payload.kind === "slack.stream.start") {
                return {
                    deliveryId: value.deliveryId,
                    seq: value.seq,
    ...
    const claimedDelivery = new ClaimedChannelDelivery(delivery, { ownerGeneration: 1, runtimeInstanceId: "rti_slack_stream_01" }, deliveryChannel, async () => ({
...
test("starts an explicit managed Slack stream with its first text chunk", async () => {
    const fixture = setup();
    try {
        await fixture.adapter.stream(fixture.target, streamChunks("", "Hello", " world"));
        expect(fixture.payloads).toEqual([
            { kind: "slack.stream.start", initialText: "Hello" },
            {
...
test("starts direct-message status before a threaded native stream", async () => {
    ...
    try {
        const renderer = fixture.adapter.createRunRenderer(fixture.target);
        await renderer.subscriber.onRunStartedEvent?.({ event: {} });
        renderer.subscriber.onTextMessageContentEvent?.({
            event: {
                type: EventType.TEXT_MESSAGE_CONTENT,
                messageId: "message-1",
...
test("uses matching legacy references for a long direct-message fallback", async () => {
    ...
    try {
        ...
        await renderer.finish?.();
        const creates = fixture.payloads.filter((payload) => payload.kind === "slack.message.create");
        const replaces = fixture.payloads.filter((payload) => payload.kind === "slack.message.replace");
