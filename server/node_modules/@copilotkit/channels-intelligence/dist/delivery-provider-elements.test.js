...
function target(provider, effect) {
    return {
        claimedDelivery: { effect },
...
test.each([
    ["slack", "teams"],
    ["teams", "slack"],
])("%s delivery rejects a %s-native element before an effect is sent", async (activeProvider, elementProvider) => {
    const effect = vi.fn();
    const ir = [
        {
            type: "raw",
            props: {
                ...
                value: elementProvider === "teams"
                    ? { type: "AdaptiveCard", version: "1.5", body: [] }
                    : [{ type: "section", text: { type: "plain_text", text: "hi" } }],
            },
        },
    ];
    await expect(makeAdapter().post(target(activeProvider, effect), ir)).rejects.toBeInstanceOf(ChannelProviderMismatchError);
