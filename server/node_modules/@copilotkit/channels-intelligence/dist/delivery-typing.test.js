...
async function deliverNoopTeamsMessage(typingDelayMs) {
...
test.each([
...
])("Teams personal-file consent %s stays system-owned", async (action, expectedKinds) => {
    const gateway = new DeliveryTestGateway();
    ...
    try {
        await gateway.deliver(preparedDelivery(`file_consent_${action}`, "teams", {
            kind: "file_consent",
            action,
            fileHandle: "fileref_personal_file_01",
        }));
        expect(gateway.packets.map((packet) => packet.payload.kind)).toEqual(expectedKinds);
