...
import { DeliveryTestGateway, preparedDelivery, } from "./delivery-test-gateway.js";
...
test("delivery handlers receive the source provider", async () => {
    ...
    try {
        await gateway.deliver(preparedDelivery("text", "slack", {
            kind: "text",
            text: "hello",
            messageRef: { id: "pref_v1_sourcePlatformText_123" },
        }));
        await gateway.deliver(preparedDelivery("interaction", "slack", {
            kind: "interaction",
            actionId: "approve",
        }));
        await gateway.deliver(preparedDelivery("reaction", "teams", {
            kind: "reaction",
            rawEmoji: "thumbs-up",
            added: true,
        ...
        await gateway.deliver(preparedDelivery("interaction-values", "teams", {
            kind: "interaction",
            actionId: "submit",
            values: { reason: "ready" },
        }));
        await gateway.deliver(preparedDelivery("welcome", "teams", { kind: "welcome" }));
        ...
        expect(identifyUser).toHaveBeenCalledWith(expect.objectContaining({
            provider: "slack",
...
test("managed text selects onMention with onMessage fallback", async () => {
    ...
    const mentionHandle = await startChannelsWithGatewayControl([mentionChannel], {
    ...
    try {
        await mentionGateway.deliver(preparedDelivery("mention-route", "teams", {
            kind: "text",
            text: "@bot hello",
    ...
    const fallbackHandle = await startChannelsWithGatewayControl([fallbackChannel], {
    ...
    try {
        await fallbackGateway.deliver(preparedDelivery("mention-fallback", "slack", {
            kind: "text",
            text: "@bot hello",
