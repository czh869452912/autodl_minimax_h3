...
test("image uploads carry an allowlisted MIME inferred from the filename", async () => {
    ...
    expect(fetch.mock.calls[0]?.[0]).toContain("operationId=response_image_upload_01");
...
test("storage retries reuse one operation identity", async () => {
    const fetch = vi
        ...
        .mockResolvedValueOnce(new Response(JSON.stringify({ handle: "file_retry_1" }), {
        status: 200,
        headers: { "content-type": "application/json" },
    }));
    const client = new ChannelDeliveryFileClient({
    ...
    await expect(client.uploadFile("delivery-1", "response_storage_retry_01", {
        bytes: new Uint8Array([1, 2, 3]),
        filename: "report.pdf",
