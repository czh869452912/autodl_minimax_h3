...
test("charge client calls the delivery-scoped runtime route", async () => {
    ...
    expect(fetch).toHaveBeenCalledExactlyOnceWith("https://api.example/api/channels/deliveries/dlv_charge_delivery/charge", {
        method: "POST",
...
test("one claimed delivery shares one charge across substantive effects", async () => {
    ...
    const channel = {
        joinReply: delivery,
        push: vi.fn(async (_event, packet) => ({
    ...
    const claimed = new ClaimedChannelDelivery(delivery, { ownerGeneration: 1, runtimeInstanceId: "rti_charge_01" }, channel, vi.fn(), undefined, undefined, new AbortController().signal, { charge });
...
test("Slack status is unmetered and does not count as provider output", async () => {
    ...
    const claimed = new ClaimedChannelDelivery(delivery, { ownerGeneration: 1, runtimeInstanceId: "rti_charge_01" }, channel, vi.fn(), undefined, undefined, new AbortController().signal, { charge });
    await claimed.effect("response_status", { kind: "slack.thread.status", status: "is thinking…" }, { charge: false });
