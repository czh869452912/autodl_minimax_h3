 ...
 */
export function parseChannelProviderStates(reply) {
    if (typeof reply !== "object" || reply === null)
        return undefined;
    const raw = reply.channels;
...
export class RealtimeGatewayUnreachableError extends Error {
     ...
     */
    constructor(endpoint, transportError, connectTimeoutMs, options) {
        super(`realtime gateway unreachable: the socket never connected to ${endpoint} ` +
            (transportError !== undefined
                ? `(${transportError})`
                : `and reported no transport error within ${connectTimeoutMs}ms`) +
            " — check that the Intelligence wsUrl points at the gateway host", options);
...
export class RealtimeGatewayJoinError extends Error {
    ...
    constructor(reason, options) {
        super(options?.message ??
            `realtime gateway session join failed: ${safeReason(reason)}`, { cause: reason });
...
function isRetryableJoinRejection(reason) {
...
export class RealtimeGatewayPushError extends Error {
    event;
    code;
    constructor(event, code, reason) {
        super(`realtime gateway session push ${event} failed: ${safeReason(reason)}`);
        this.event = event;
        this.code = code;
 ...
 */
const NON_RETRYABLE_TRANSPORT_CODES = new Set([
    "ENOTFOUND",
    "EAI_NONAME",
]);
/**
 ...
 */
function describeRefusedUpgrade(result) {
    if (result.status === undefined)
        return result.text;
    if (result.status >= 500) {
        return (`the gateway host answered HTTP ${result.status}, so it is reachable but not healthy — ` +
            "it may be restarting");
    }
    return (`the gateway host answered HTTP ${result.status} but the WebSocket handshake was refused, ` +
 ...
 */
async function diagnoseEndpoint(wsUrl, timeoutMs, fetchImpl = globalThis.fetch) {
    ...
    try {
        ...
        return {
            text: `the host answered HTTP ${response.status} for ${httpUrl}, so it resolves and accepts ` +
 ...
 */
export async function connectRealtimeGateway(config) {
    if (!Number.isSafeInteger(config.projectId) || config.projectId <= 0) {
        throw new Error("connectRealtimeGateway: projectId must be a positive integer");
    }
    ...
    const transport = config.webSocket ??
        globalThis.WebSocket;
    if (!transport) {
        throw new Error("connectRealtimeGateway: no WebSocket available — pass config.webSocket or run on Node 22+");
    }
    const socket = new Socket(config.wsUrl, {
        authToken: config.apiKey,
        transport: transport,
    });
    const channel = socket.channel("control", config.join);
    ...
    let controlJoinReply;
    let lastTransportError;
    let lastTransportCode;
    let lastTransportRaw;
    ...
    let settleConnect;
    let failConnect;
    const initialConnect = new Promise((resolve, reject) => {
    ...
    const startDiagnosis = () => {
        if (diagnosis === undefined) {
            diagnosis = diagnoseEndpoint(config.wsUrl, probeTimeoutMs, config.diagnosticFetch);
    ...
    const probeOutage = () => {
        ...
        outageProbe = diagnoseEndpoint(config.wsUrl, probeTimeoutMs, config.diagnosticFetch).then((result) => {
            if (result === undefined || episode !== outageEpisode)
                return;
    ...
    const failUnreachable = () => {
        ...
        void (async () => {
            ...
            const probed = lastTransportCode !== undefined
                ? await diagnosis
                : await startDiagnosis();
            const cause = probed?.cause ?? lastTransportRaw;
            const retryable = !(lastTransportCode !== undefined &&
                NON_RETRYABLE_TRANSPORT_CODES.has(lastTransportCode)) &&
                (probed === undefined ||
                    (!probed.nonRetryable &&
                        (probed.status === undefined || probed.status >= 500)));
            failConnect(new RealtimeGatewayUnreachableError(endpoint, probed?.text ?? lastTransportError, connectTimeoutMs, {
    ...
    socket.onError((error, _transport, establishedConnections) => {
        ...
        if (lastTransportCode !== undefined &&
            NON_RETRYABLE_TRANSPORT_CODES.has(lastTransportCode)) {
            failUnreachable();
            return;
        }
    ...
    channel.on("delivery_invitation", (payload) => {
        ...
        if (pendingInvitations.length >= maxPendingInvitations) {
            if (!initialJoinSettled) {
                ...
                closingIntentionally = true;
                socket.disconnect();
                failConnect(new Error(`realtime gateway sent more than ${maxPendingInvitations} invitations before control setup completed`));
            }
            return;
        }
    ...
    joinPush
        ...
        if (!initialJoinSettled) {
            ...
            closingIntentionally = true;
            socket.disconnect();
            failConnect(new RealtimeGatewayJoinError(undefined, {
    ...
    const wrapChannel = (joinedChannel, joinReply) => ({
        joinReply,
        push: (event, payload) => new Promise((resolve, reject) => {
            joinedChannel
                .push(event, payload, timeout)
                .receive("ok", (reply) => resolve(reply))
                .receive("error", (reason) => reject(new RealtimeGatewayPushError(event, extractReasonCode(reason), reason)))
                .receive("timeout", () => reject(new Error(`realtime gateway session push ${event} timed out`)));
        }),
        on: (event, handler) => {
            if (joinedChannel === channel && event === "delivery_invitation") {
                invitationHandlers.push(handler);
                const buffered = pendingInvitations.splice(0, pendingInvitations.length);
        ...
        join: async (topic, payload) => {
            const child = socket.channel(topic, payload);
            try {
    ...
    const notifyClose = () => {
        if (closingIntentionally || closeFired)
            return;
        closeFired = true;
    ...
    let errorWithoutCloseTimer;
    const clearErrorWithoutCloseTimer = () => {
        if (errorWithoutCloseTimer === undefined)
            return;
        clearTimeout(errorWithoutCloseTimer);
        errorWithoutCloseTimer = undefined;
    };
    const armErrorWithoutCloseTimer = () => {
        if (closingIntentionally || errorWithoutCloseTimer !== undefined)
            return;
...
function joinPhoenixChannel(channel, timeoutMs) {
    return new Promise((resolve, reject) => {
        channel
            .join(timeoutMs)
            .receive("ok", (reply) => resolve(reply))
            .receive("error", (reason) => reject(new Error(`realtime gateway delivery join failed: ${safeReason(reason)}`)))
            .receive("timeout", () => reject(new Error("realtime gateway delivery join timed out")));
    });
}
/**
 ...
 */
function describeEndpoint(wsUrl) {
    try {
