...
function resolveModel(spec, apiKey) {
	...
	switch (provider) {
		case "openai": return (0, _ai_sdk_openai.createOpenAI)({
			apiKey: apiKey || process.env.OPENAI_API_KEY,
			baseURL: process.env.OPENAI_BASE_URL
		})(model);
		case "anthropic": return (0, _ai_sdk_anthropic.createAnthropic)({
			apiKey: apiKey || process.env.ANTHROPIC_API_KEY,
			baseURL: process.env.ANTHROPIC_BASE_URL
		})(model);
		case "google":
		case "gemini":
		case "google-gemini": return (0, _ai_sdk_google.createGoogleGenerativeAI)({
			apiKey: apiKey || process.env.GOOGLE_API_KEY,
			baseURL: process.env.GOOGLE_GENERATIVE_AI_BASE_URL
		})(model);
		case "minimax": return (0, _ai_sdk_openai.createOpenAI)({
			name: "minimax",
...
function convertJsonSchemaToZodSchema(jsonSchema, required) {
	...
	if (Array.isArray(unionVariants) && unionVariants.length > 0) {
		if (unionVariants.length === 1) return convertJsonSchemaToZodSchema(unionVariants[0], required);
		const variantSchemas = unionVariants.map((variant) => convertJsonSchemaToZodSchema(variant, true));
		const schema = zod.z.union(variantSchemas).describe(jsonSchema.description ?? "");
...
\`\`\`json\n${JSON.stringify(input.state, null, 2)}\n\`\`\`\n`);
			...
			(async () => {
				...
				try {
					...
					const normalizeStateEvent = require_state_delta.createStateEventNormalizer(input.state);
