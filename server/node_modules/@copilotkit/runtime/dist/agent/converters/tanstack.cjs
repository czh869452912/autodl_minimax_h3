...
async function* convertTanStackStream(stream, abortSignal, pendingInterrupts, initialState) {
	...
	const normalizeStateEvent = require_state_delta.createStateEventNormalizer(initialState);
	...
	for await (const chunk of stream) {
		...
		if (type === "TEXT_MESSAGE_CONTENT" && raw.delta != null) {
			...
			if (toolName === "AGUISendStateSnapshot" && parsedContent && typeof parsedContent === "object" && "snapshot" in parsedContent) {
				const stateSnapshotEvent = {
					type: _ag_ui_client.EventType.STATE_SNAPSHOT,
					snapshot: parsedContent.snapshot
				};
				for (const event of normalizeStateEvent(stateSnapshotEvent)) yield event;
			}
			if (toolName === "AGUISendStateDelta" && parsedContent && typeof parsedContent === "object" && "delta" in parsedContent) {
				const stateDeltaEvent = {
					type: _ag_ui_client.EventType.STATE_DELTA,
					delta: parsedContent.delta
				};
				for (const event of normalizeStateEvent(stateDeltaEvent)) yield event;
			}
