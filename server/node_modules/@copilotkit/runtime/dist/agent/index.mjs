...
function resolveModel(spec, apiKey) {
	...
	switch (provider) {
		...
		case "google":
		case "gemini":
		case "google-gemini": return createGoogleGenerativeAI({
			apiKey: apiKey || process.env.GOOGLE_API_KEY,
			baseURL: process.env.GOOGLE_GENERATIVE_AI_BASE_URL
		})(model);
		case "minimax": return createOpenAI({
			name: "minimax",
...
function convertJsonSchemaToZodSchema$1(jsonSchema, required) {
	...
	if (Array.isArray(unionVariants) && unionVariants.length > 0) {
		if (unionVariants.length === 1) return convertJsonSchemaToZodSchema$1(unionVariants[0], required);
		const variantSchemas = unionVariants.map((variant) => convertJsonSchemaToZodSchema$1(variant, true));
		const schema = z.union(variantSchemas).describe(jsonSchema.description ?? "");
							...
							case "tool-result": {
								...
								if (toolName === "AGUISendStateSnapshot" && toolResult && typeof toolResult === "object") {
									...
									if (snapshot !== void 0) {
										...
										for (const event of normalizeStateEvent(stateSnapshotEvent)) subscriber.next(event);
									}
									...
									if (delta !== void 0) {
										...
										for (const event of normalizeStateEvent(stateDeltaEvent)) subscriber.next(event);
									}
								}
		...
		return new Observable((subscriber) => {
			...
			(async () => {
				try {
					...
					switch (config.type) {
						case "aisdk":
							events = convertAISDKStream((await config.factory(factoryCtx)).fullStream, controller.signal, pendingInterrupts, input.state);
							break;
						case "tanstack":
							events = convertTanStackStream(await config.factory(factoryCtx), controller.signal, pendingInterrupts, input.state);
							break;
						case "custom":
