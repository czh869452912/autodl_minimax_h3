...
//#region src/v1-deprecated/lib/runtime/mcp-tools-utils.ts
/**
...
function extractParametersFromSchema(toolOrSchema) {
	...
	for (const paramName in properties) if (Object.prototype.hasOwnProperty.call(properties, paramName)) {
		...
		if (paramDef.enum && Array.isArray(paramDef.enum)) {
			...
			description = description + (description ? " " : "") + `Allowed values: ${enumDisplay}`;
		}
		let attributes;
		if (type === "object" && paramDef.properties) {
			...
			attributes = extractParametersFromSchema({ parameters: {
				properties: paramDef.properties,
				required: paramDef.required || []
			} });
		}
		if (type === "array" && paramDef.items?.type === "object" && paramDef.items?.properties) attributes = extractParametersFromSchema({ parameters: {
		...
		if (type === "string" && enumValues) param.enum = enumValues;
		if (attributes && attributes.length > 0) {
			param.attributes = attributes;
			if (type === "array") param.type = "object[]";
		}
...
//#endregion
export { convertMCPToolsToActions, extractParametersFromSchema, generateMcpToolInstructions };
//# sourceMappingURL=mcp-tools-utils.mjs.map