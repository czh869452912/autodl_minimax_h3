...
//#region src/v1-deprecated/service-adapters/openai/openai-assistant-adapter.ts
var OpenAIAssistantAdapter = class {
	...
	async submitUserMessage(threadId, messages, actions, eventSource, forwardedParameters) {
		...
		const userMessage = messages.map((m) => convertMessageToOpenAIMessage(m, { keepSystemRole: this.keepSystemRole })).map(convertSystemMessageToAssistantAPI).at(-1);
		...
		const tools = [
			...actions.map(convertActionInputToOpenAITool),
...
//#endregion
export { OpenAIAssistantAdapter };
//# sourceMappingURL=openai-assistant-adapter.mjs.map