...
//#region src/v1-deprecated/service-adapters/anthropic/utils.ts
...
*/
function coalesceConsecutiveSameRoleMessages(messages) {
	const result = [];
	for (const msg of messages) {
		const prev = result[result.length - 1];
		if (prev && prev.role === msg.role && Array.isArray(prev.content) && Array.isArray(msg.content)) prev.content.push(...msg.content);
		else result.push({
			...msg,
...
//#endregion
export { coalesceConsecutiveSameRoleMessages, convertActionInputToAnthropicTool, convertMessageToAnthropicMessage, limitMessagesToTokenCount };
