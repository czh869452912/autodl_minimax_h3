...
async function defaultActivateChannel(config, channel, importChannelsIntelligence = () => import(CHANNELS_INTELLIGENCE_SPECIFIER), log, services) {
	...
	return mod.startChannelsOverRealtimeGateway([channel], {
		...
		...log ? { log } : {},
		runCanonical: (args) => runCanonicalChannelAgent(services.runner, services.intelligence, services.lockTtlSeconds ?? 20, services.lockHeartbeatIntervalSeconds ?? 15, args, services.lockKeyPrefix, services.learning),
...
async function runCanonicalChannelAgent(runner, intelligence, lockTtlSeconds, lockHeartbeatIntervalSeconds, args, lockKeyPrefix, learning) {
	const learningContainerId = await require_learning.resolveLearningContainerId(learning, {
		surface: "channel",
...
var ChannelManager = class {
	...
	constructor(args) {
		...
		this.activateChannel = args.activateChannel ?? ((config, channel) => defaultActivateChannel(config, channel, void 0, this.log, this.runner ? {
			...
			...this.learning !== void 0 ? { learning: this.learning } : {},
	...
	registerConnectionObserver(name, entry) {
		entry.handle?.onStateChange?.((state, detail) => {
			...
			if (state === "reconnecting") {
				...
				if (cause !== void 0) entry.downCause = cause;
				...
				this.captureChannelTelemetry("oss.runtime.channel_session_dropped", {
					...detail?.reason !== void 0 ? { reason: detail.reason } : {},
				...
				entry.downCause = void 0;
				this.log?.(`channel "${name}" managed session back online`);
				if (downSince !== void 0) this.captureChannelTelemetry("oss.runtime.channel_session_recovered", { downForMs: Date.now() - downSince });
	...
	*/
	captureChannelTelemetry(event, props) {
		try {
			require_telemetry_client.default.capture(event, props).catch(() => {});
		} catch {}
	}
	...
	*/
	startReconnectLog(name, entry) {
		...
		const timer = setTimeout(() => {
			...
			this.log?.(`channel "${name}" managed session still down after ${this.downFor(entry)}; Phoenix is retrying` + (entry.downCause !== void 0 ? ` — ${entry.downCause}` : ""));
