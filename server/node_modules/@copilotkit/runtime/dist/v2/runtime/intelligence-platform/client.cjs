...
var CopilotKitIntelligence = class {
	...
	*/
	async getInspectorMetadata() {
		...
		const timeoutError = /* @__PURE__ */ new Error("Intelligence inspector metadata request timed out");
		let timeoutId;
		...
		try {
			const response = await Promise.race([fetch(`${this.#apiUrl}${path}`, {
				method: "GET",
			...
			if (!response.ok) {
				...
				throw new PlatformRequestError(`Intelligence platform error ${response.status}`, response.status);
			}
			const body = await Promise.race([response.text(), timeout]);
			return (0, _copilotkit_shared.parseInspectorMetadataV1)(JSON.parse(body));
		} catch (error) {
			if (error === timeoutError) _copilotkit_shared.logger.warn({
				path,
				timeoutMs: INSPECTOR_METADATA_REQUEST_TIMEOUT_MS
