	...
	*/
	...
	createRunObservable(request, startupBoundary) {
		...
		return new rxjs.Observable((observer) => {
			...
			channel.on(_copilotkit_shared.AG_UI_CHANNEL_EVENT, (payload) => {
				if (this.isCurrentThreadState(threadId, state) && payload.type === _ag_ui_client.EventType.CUSTOM && payload.name === "stop") this.stop({
					threadId,
					runId: state.runId
				});
			});
	...
	async executeAgentRun(request, state, threadId, onRunError) {
		...
		try {
			...
			if (!this.isCurrentThreadState(threadId, state)) return;
			ensureRunStarted();
			const existingError = currentEvents.find((event) => event.type === _ag_ui_client.EventType.RUN_ERROR);
			...
			if (!this.isCurrentThreadState(threadId, state)) return;
			ensureRunStarted();
			const appended = (0, _copilotkit_shared.finalizeRunEvents)(currentEvents, { stopRequested: state.stopRequested });
