...
describe("SlackAdapter.postEphemeral", () => {
    it("posts a native ephemeral message (usedFallback=false)", async () => {
        ...
        const res = await adapter.postEphemeral({ channel: "C1", threadTs: "1.0" }, { id: "U1", kind: "human" }, ir, { fallbackToDM: false });
        expect(res).toMatchObject({ ok: true, usedFallback: false });
        expect(postEphemeral).toHaveBeenCalledWith(expect.objectContaining({ channel: "C1", user: "U1", thread_ts: "1.0" }));
    });
    it("accepts a string user id", async () => {
        ...
        expect(res).toMatchObject({ ok: true, usedFallback: false });
        expect(postEphemeral).toHaveBeenCalledWith(expect.objectContaining({ channel: "C2", user: "U2" }));
        // No thread_ts when target has none.
        const args = postEphemeral.mock.calls[0][0];
    ...
    it("omits thread_ts when target has none", async () => {
        ...
        // @ts-expect-error inject stub
        adapter.client = { chat: { postEphemeral } };
        await adapter.postEphemeral({ channel: "C3" }, { id: "U3", kind: "human" }, [], {
            fallbackToDM: false,
        });
        const args = postEphemeral.mock.calls[0][0];
        expect(args["thread_ts"]).toBeUndefined();
    ...
    it("ignores fallbackToDM and always posts natively (usedFallback always false)", async () => {
        ...
        // @ts-expect-error inject stub
        adapter.client = { chat: { postEphemeral } };
        const res = await adapter.postEphemeral({ channel: "C4" }, { id: "U4", kind: "human" }, [], { fallbackToDM: true });
        expect(res).toMatchObject({ ok: true, usedFallback: false });
    });
    it("returns { ok: false, error } when the Slack API throws", async () => {
        ...
        // @ts-expect-error inject stub
        adapter.client = { chat: { postEphemeral } };
        const res = await adapter.postEphemeral({ channel: "C5" }, { id: "U5", kind: "human" }, [], { fallbackToDM: false });
