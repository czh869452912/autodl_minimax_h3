...
describe("createRunRenderer", () => {
    ...
    it("posts placeholder with thread_ts for thread replies, none for DMs (only on first content)", async () => {
        ...
        await dm.onTextMessageEndEvent({ event: { messageId: "m1" } });
    ...
    it("interrupt: an on_interrupt custom event sets getPendingInterrupt", async () => {
        const fake = makeFakeClient();
        const { subscriber: sub, getPendingInterrupt, clearPendingInterrupt, } = createRunRenderer({
            transport: fake.transport,
            target: { channel: "C1", threadTs: "100.0" },
        });
        sub.onCustomEvent({
    ...
    it("logs a failed best-effort error notice at debug level", async () => {
        ...
        expect(debug).toHaveBeenCalledWith("[slack-renderer] error notice failed:", failure);
    ...
    it("does not bleed buffers between messages with different ids", async () => {
        ...
        await sub.onTextMessageEndEvent({ event: { messageId: "a" } });
        await sub.onTextMessageEndEvent({ event: { messageId: "b" } });
...
describe("createRunRenderer — native status mode", () => {
    ...
    it("logs a failed best-effort status update at debug level", async () => {
        ...
        expect(debug).toHaveBeenCalledWith("[slack-renderer] setStatus failed:", failure);
        expect(error).not.toHaveBeenCalled();
        debug.mockRestore();
        error.mockRestore();
    });
    ...
    it("clears the status once a reply is posted", async () => {
        ...
        await sub.onTextMessageEndEvent({ event: { messageId: "m" } });
