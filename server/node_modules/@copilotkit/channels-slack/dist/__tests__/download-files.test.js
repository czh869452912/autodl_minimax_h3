...
function fakeFetch(bodyByUrl) {
    return vi.fn(async (url) => {
        ...
        return {
            ok: e.ok ?? true,
            status: e.status ?? 200,
            arrayBuffer: async () => (e.bytes ?? Buffer.from("")).buffer.slice((e.bytes ?? Buffer.from("")).byteOffset, (e.bytes ?? Buffer.from("")).byteOffset +
...
describe("buildFileContentParts", () => {
    ...
    ])("passes $mimetype through as a $expected part", async ({ name, mimetype, expected }) => {
    ...
    it("skips on a failed download with a note", async () => {
        vi.stubGlobal("fetch", fakeFetch({ [img.url_private]: { ok: false, status: 403 } }));
    ...
    it("caps the number of files processed", async () => {
        ...
        vi.stubGlobal("fetch", fakeFetch(Object.fromEntries(files.map((f) => [f.url_private, { bytes: Buffer.from("x") }]))));
