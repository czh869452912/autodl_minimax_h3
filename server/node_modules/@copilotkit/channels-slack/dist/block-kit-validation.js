...
export function validateSlackBlockKit(blocks) {
...
function validateCard(card, path) {
    ...
    if (card.hero_image === undefined &&
        card.title === undefined &&
        card.actions === undefined &&
        card.body === undefined) {
        throw new TypeError(`${path}: card requires hero_image, title, actions, or body`);
    }
    if (card.actions !== undefined) {
        if (!Array.isArray(card.actions))
            invalidField(`${path}/actions`);
...
function validateCarousel(carousel, path) {
    rejectUnknownFields(carousel, CAROUSEL_FIELDS, path);
    if (!Array.isArray(carousel.elements) ||
    ...
    carousel.elements.forEach((card, index) => {
        ...
        if (!isRecord(card) || card.type !== "card")
            invalidField(cardPath);
...
function invalidField(path, expected) {
    throw new TypeError(`invalid field at ${path}${expected ? `; ${expected}` : ""}`);
}
