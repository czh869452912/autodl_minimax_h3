...
export function validateSlackDataVisualization(value, path) {
    ...
    switch (chart.type) {
        ...
        case "line":
            ...
            return;
        default:
            throw new Error(`${blockPath}.chart.type must be pie, bar, area, or line.`);
    }
}
function validatePieChart(chart, path) {
...
function validateSeriesChart(chart, path) {
    const series = assertArray(chart.series, `${path}.series`, 1, 12);
    const names = [];
    const dataBySeries = series.map((value, index) => {
    ...
    const categoryValues = assertArray(axis.categories, `${path}.axis_config.categories`, 1, 20);
    const categories = categoryValues.map((category, index) => assertString(category, `${path}.axis_config.categories[${index}]`, 20));
    ...
    for (const current of dataBySeries) {
        ...
        if (labels.length !== categories.length ||
            ...
            throw new Error(`${current.path}.data labels must match axis_config.categories exactly.`);
        }
    }
}
...
function assertString(value, path, maximum) {
