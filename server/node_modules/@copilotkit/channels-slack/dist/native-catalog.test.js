...
import { SLACK_BLOCK_MANIFEST, SLACK_ELEMENT_MANIFEST, SLACK_NATIVE_MANIFEST, SLACK_OBJECT_MANIFEST, SLACK_UNTYPED_OBJECTS, } from "./native-manifest.js";
...
test("Slack exposes all 19 message-valid blocks from its manifest", () => {
    ...
    expect(Object.keys(Slack.Block).sort()).toEqual(SLACK_BLOCK_MANIFEST.map(([name]) => name).sort());
    expect(Object.keys(Slack.Element).sort()).toEqual(SLACK_ELEMENT_MANIFEST.map(([name]) => name).sort());
    expect(Object.keys(Slack.Object).sort()).toEqual(SLACK_OBJECT_MANIFEST.map(([name]) => name).sort());
});
test("every Slack catalog entry serializes its fixed discriminator", () => {
    for (const entry of SLACK_NATIVE_MANIFEST) {
        const node = createNativeNode("slack", entry.kind, entry.type, requiredProps(entry.type));
...
function requiredProps(type) {
    if (type === "actions" || type === "context")
        return { elements: [] };
...
test("an image accepts either an external URL or a Slack-hosted file", () => {
    ...
    expect(serializeSlackNativeNode(external).image_url).toBe("https://picsum.photos/400/300");
    ...
    expect(() => serializeSlackNativeNode(neither)).toThrow(/requires image_url or slack_file/);
});
