...
import { markdownToRichTextRuns, needsRichText, truncateRuns, } from "../markdown-to-rich-text.js";
...
export function renderBlockKit(ir) {
    ...
    const dataVisualizations = blocks.filter((block) => String(block.type) === "data_visualization").length;
    if (dataVisualizations > 2) {
        throw new Error(`Slack native JSX rendered ${dataVisualizations} data visualization blocks; the message limit is 2.`);
    }
    if (native && blocks.length > SLACK_LIMITS.blocksPerMessage) {
        throw new Error(`Slack native JSX rendered ${blocks.length} blocks; the message limit is ${SLACK_LIMITS.blocksPerMessage}.`);
    }
...
function containsNativeNode(nodes) {
    return nodes.some((node) => isNativeNode(node) ||
        containsNativeNode(channelChildren(node.props.children)));
}
function channelChildren(value) {
    if (Array.isArray(value))
        return value.filter(isChannelNode);
...
function renderNode(node, out) {
    if (isNativeNode(node)) {
        if (node.props.nativeKind !== "block" && node.props.nativeKind !== "raw") {
            throw new Error(`Slack.${node.props.nativeType}: a top-level message child must be a block.`);
        }
        out.push(serializeSlackNativeNode(node));
        return;
    }
