 ...
 */
export function markdownToRichTextRuns(markdown) {
    if (!markdown)
        return [];
 ...
 */
export function needsRichText(runs) {
    return runs.some((run) => run.type === "link" || Object.keys(run.style ?? {}).length > 0);
}
/**
 ...
 */
export function truncateRuns(runs, max) {
...
function visibleText(run) {
    return run.text ?? "";
}
function withText(run, text) {
    return { ...run, text };
}
function tokenize(input, inherited) {
    const runs = [];
    const pushText = (text) => {
        if (text.length > 0)
            runs.push(styled({ type: "text", text }, inherited));
    };
    let last = 0;
    ...
    while ((match = re.exec(input)) !== null) {
        ...
        if (url !== undefined) {
            const inner = label === undefined || label === ""
                ? undefined
            ...
            runs.push(styled({ type: "text", text: code }, { ...inherited, code: true }));
        }
