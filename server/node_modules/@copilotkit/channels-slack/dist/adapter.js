...
export class SlackAdapter {
    ...
    async start(sink) {
        ...
        attachSlackListener({
            ...
            onTurn: async (turn) => {
                ...
                const actor = turn.senderUserId
                    ? { id: turn.senderUserId, kind: "human" }
                    : { id: "", kind: "unknown" };
                await sink.onTurn({
                    conversationKey,
                    ...
                    identityContext: this.identityContext({
                        conversationKey,
                        conversationKind: turn.conversation.scope === DM_SCOPE ? "direct" : "thread",
                        trigger: "message",
            ...
            onCommand: async (cmd) => {
                ...
                const actor = cmd.senderUserId
                    ? { id: cmd.senderUserId, kind: "human" }
                    : { id: "", kind: "unknown" };
                await sink.onCommand({
                    ...
                    identityContext: this.identityContext({
                        conversationKey,
                        conversationKind: cmd.conversation.scope === DM_SCOPE ? "direct" : "thread",
                        trigger: "command",
                        eventId: cmd.eventId,
                        actor,
        ...
        this.app.event("reaction_added", async ({ event }) => {
            ...
            if (event.user)
                evt.actor = { id: event.user, kind: "human" };
        ...
        this.app.view(/.*/, async ({ ack, body, view }) => {
            const actor = body.user?.id
                ? { id: body.user.id, kind: "human", name: body.user.name }
                : undefined;
    ...
    nativeTransport(t, onFirstTs) {
        ...
        const startStream = async (markdownText) => {
            const args = {
                channel: t.channel,
                thread_ts: threadTs,
