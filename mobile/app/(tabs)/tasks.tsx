...
import { getTask } from '../../src/tasks/api';
...
export default function TasksScreen() {
  ...
  const load = useCallback(async () => { const current = await taskStore.list(); setTasks(current); const settings = await readSettings(); if (!settings.token) return; for (const task of current.filter((item) => item.status === 'QUEUED' || item.status === 'RUNNING')) { try { const next = await getTask(settings.token, task); await taskStore.upsert(next); } catch {} } for (const task of await taskStore.list()) { if (task.status === 'SUCCESS' && task.videoUrl && !task.localUri) { try { const target = `${FileSystem.documentDirectory || ''}media/${task.id}.mp4`; await FileSystem.makeDirectoryAsync(`${FileSystem.documentDirectory || ''}media`, { intermediates: true }); const result = await FileSystem.downloadAsync(task.videoUrl, target); await taskStore.upsert({ ...task, localUri: result.uri, updatedAt: Date.now() }); } catch {} } } setTasks(await taskStore.list()); }, []);
