...
export default function GalleryScreen() {
  ...
  const items = useMemo(() => demoAssets.filter((asset) => `${asset.title} ${asset.prompt} ${asset.taskId}`.toLowerCase().includes(query.toLowerCase())), [query]);
  return <View style={styles.container}>
    ...
    <FlatList data={items} numColumns={2} keyExtractor={(item) => item.id} columnWrapperStyle={styles.row} contentContainerStyle={styles.list} renderItem={({ item }) => <GalleryCard asset={item} onPress={() => setSelected(item)} />} ListEmptyComponent={<Text style={styles.empty}>暂无视频作品</Text>} />
