...
export function jobRecordToTaskProjection(job: JobRecord, values: ArtifactRecord[] = []): TaskRecord { const input = job.inputSnapshot; const video = values.find((item) => item.kind === 'video'); return { id: job.id, prompt: String(input.prompt ?? ''), resolution: String(input.resolution ?? ''), duration: Number(input.duration ?? 0), seed: typeof input.seed === 'string' ? input.seed : undefined, images: input.images as TaskRecord['images'], audios: input.audios as TaskRecord['audios'], status: job.status === 'SUCCEEDED' ? 'SUCCESS' : job.status === 'FAILED' ? 'FAILED' : job.status === 'CANCELLED' ? 'CANCELLED' : job.status === 'RUNNING' ? 'RUNNING' : 'QUEUED', videoUrl: video?.uri, createdAt: job.createdAt, updatedAt: job.updatedAt }; }
