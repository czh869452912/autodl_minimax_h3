...
function status(value: TaskRecord['status']): JobStatus { return value === 'SUCCESS' ? 'SUCCEEDED' : value === 'PARTIAL_SUCCESS' ? 'PARTIAL_SUCCEEDED' : value === 'FAILED' ? 'FAILED' : value === 'CANCELLED' ? 'CANCELLED' : value === 'UNKNOWN' ? 'UNKNOWN' : value === 'RUNNING' ? 'RUNNING' : 'QUEUED'; }
