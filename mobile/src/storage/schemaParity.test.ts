...
test.each([5, 6, 7, 8])('frozen v%s upgrades to fresh schema including constraints and projection triggers', version => {
  ...
  try {
    ...
    expect(APP_SCHEMA_VERSION).toBe(10);
    const v10Names = new Set(['media_asset_tombstones', 'idx_media_tombstones_task', 'media_assets_respect_deletion', 'media_refs_respect_deletion', 'media_deliveries_respect_deletion']);
...
test('frozen v9 migrates deletion guards without losing historical tables', () => {
  ...
  try {
    ...
    expect(db.getFirstSync('PRAGMA user_version')).toEqual({ user_version: 10 });
