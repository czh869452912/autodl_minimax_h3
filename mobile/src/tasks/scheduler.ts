...
export async function withSchedulerLease<T>(
  ...
  if (db?.getFirstSync && db.runSync) {
    ...
    db.runSync('INSERT OR REPLACE INTO app_scheduler_leases (lease_key, owner, expires_at) VALUES (?, ?, ?)', key, `${timestamp}-${Math.random()}`, timestamp + ttlMs);
  } else {
  ...
  try {
    ...
    if (db?.runSync) db.runSync('DELETE FROM app_scheduler_leases WHERE lease_key = ?', key);
