...
import { ListAction } from '../ui/ListPageUI';
...
export function DownloadNetworkHelp({ error }: { error?: string }) {
  ...
  return <>
    <View style={{ marginTop: 12, alignSelf: 'flex-start' }}><ListAction secondary icon="info" label="下载网络排查" onPress={() => setVisible(true)} /></View>
    <Modal visible={visible} transparent animationType="slide" onRequestClose={() => setVisible(false)}>
      <View style={{ flex: 1, backgroundColor: COLORS.scrim, justifyContent: 'center', padding: 24 }}>
        <ScrollView style={{ maxHeight: '85%', backgroundColor: COLORS.background, borderRadius: 16 }} contentContainerStyle={{ padding: 24, gap: 16 }}>
          ...
          <Text style={{ color: COLORS.textMuted, lineHeight: 22 }}>“疑似 Fake-IP”表示下载域名返回了保留地址，不代表已确认 VPN 故障。域名解析失败和普通连接失败也可能由断网或 DNS 服务不可用引起。</Text>
          <Text style={{ color: COLORS.textMuted, lineHeight: 22 }}>使用代理时，请让实际下载域名获得真实 DNS 答案。仅将流量设为 DIRECT 不一定有效。下载可能跳转到动态 CDN，不能只配置 autodl.art。</Text>
          <Text style={{ color: COLORS.textMuted, lineHeight: 22 }}>Mihomo：先检查 fake-ip-filter-mode；blacklist 模式下将实际下载域名加入 fake-ip-filter。whitelist / rule 模式含义不同，请按当前客户端说明调整。</Text>
          <Text style={{ color: COLORS.textMuted, lineHeight: 22 }}>sing-box 1.12+：检查 DNS 规则，使下载域名使用非 FakeIP DNS 服务器。旧版 DNS 配置不同，请查阅对应版本说明。</Text>
          <Text style={{ color: COLORS.textMuted, lineHeight: 22 }}>在代理客户端确认实际失败连接的域名；不要分享带查询参数的完整下载链接。配置生效后返回任务，点击“重试下载”。私有或其他保留地址仍会被拒绝。</Text>
          ...
          <ListAction label="关闭排查说明" onPress={() => setVisible(false)} />
        </ScrollView>
      </View>
    </Modal>
  </>;
