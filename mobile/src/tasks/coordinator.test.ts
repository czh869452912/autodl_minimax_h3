...
function setup(tasks: TaskRecord[], jobs: JobRecord[]) {
  const taskStore = { list: jest.fn(async () => tasks), upsert: jest.fn(async (value: TaskRecord) => { const index = tasks.findIndex((item) => item.id === value.id); if (index >= 0) tasks[index] = value; }), };
