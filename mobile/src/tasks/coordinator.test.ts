...
function setup(tasks: TaskRecord[], jobs: JobRecord[]) {
  const taskStore = { list: jest.fn(async () => tasks), listActive: jest.fn(async () => tasks.filter((item) => ['QUEUED', 'RUNNING', 'UNKNOWN'].includes(item.status))), upsert: jest.fn(async (value: TaskRecord) => { const index = tasks.findIndex((item) => item.id === value.id); if (index >= 0) tasks[index] = value; }), };
