...
export function taskToMediaAsset(task: TaskRecord): MediaAsset | null {
  ...
  return { id: task.id, taskId: task.id, title: task.prompt.slice(0, 48) || task.id, prompt: task.prompt, sourceUrl: task.videoUrl || '', localPath, posterPath: task.thumbnailUrl, mimeType: 'video/mp4', durationMs: task.duration * 1000, status, createdAt: task.createdAt, updatedAt: task.updatedAt };
}
