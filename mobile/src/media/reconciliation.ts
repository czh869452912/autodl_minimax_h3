...
async function listTaskPage(db: SQLiteDatabase, limit: number, cursor?: ReconciliationCursor): Promise<TaskRow[]> {
  ...
  return rows.length ? rows : await db.getAllAsync<TaskRow>(`${select} ORDER BY updated_at ASC,id ASC LIMIT ?`, limit);
}
...
export async function reconcileMediaState(options: {
  ...
  const tasks = await listTaskPage(options.db, limit, await readCursor(options.db));
  let repaired = 0;
  let staleFiles = 0;

  for (const task of tasks) {
    let changed = false;
    const job = await options.db.getFirstAsync<{ workflow_id: string }>('SELECT workflow_id FROM workflow_jobs WHERE id=? LIMIT 1', task.id);
    const workflowId = job?.workflow_id ?? 'recovered';
    let artifacts = await options.db.getAllAsync<ArtifactRow>('SELECT * FROM workflow_artifacts WHERE job_id=? ORDER BY id', task.id);
    if (artifacts.length === 0) {
      const operationRows = await options.db.getAllAsync<{ payload_json: string }>(
      ...
      for (const row of operationRows) {
        ...
        const result = await options.db.runAsync(
      ...
      artifacts = await options.db.getAllAsync<ArtifactRow>('SELECT * FROM workflow_artifacts WHERE job_id=? ORDER BY id', task.id);
    }
    ...
    for (const row of artifacts) {
      ...
      changed = await insertAsset(options.db, task, artifact, workflowId, now) || changed;
    }

    let assets = await options.db.getAllAsync<AssetRow>("SELECT id,local_path,source_url,status FROM media_assets WHERE task_id=? AND kind='video' ORDER BY updated_at DESC,id ASC", task.id);
    if (assets.length === 0 && (task.local_uri || task.video_url)) {
      changed = await insertAsset(options.db, task, {
        id: 'recovered-primary-video', jobId: task.id, kind: 'video', uri: task.video_url ?? undefined, mime: 'video/mp4',
      }, workflowId, now) || changed;
      assets = await options.db.getAllAsync<AssetRow>("SELECT id,local_path,source_url,status FROM media_assets WHERE task_id=? AND kind='video' ORDER BY updated_at DESC,id ASC", task.id);
    }
    ...
    for (const uri of localUris) {
      ...
      await options.db.runAsync("UPDATE tasks SET local_uri=NULL,download_state=?,download_error=?,download_progress=NULL,updated_at=MAX(updated_at,?) WHERE id=? AND local_uri=?", hasRemote ? 'IDLE' : 'DOWNLOAD_FAILED', hasRemote ? null : 'ARTIFACT_SOURCE_MISSING', now, task.id, uri);
      await options.db.runAsync("UPDATE media_assets SET local_path=NULL,status=?,updated_at=? WHERE task_id=? AND local_path=?", hasRemote ? 'queued' : 'failed', now, task.id, uri);
    }
    ...
    if (task.export_state === 'EXPORTED' && task.gallery_uri && assets[0]) {
      ...
      changed = Number(result.changes ?? 0) === 1 || changed;
      await options.db.runAsync("UPDATE media_assets SET export_status='EXPORTED',updated_at=MAX(updated_at,?) WHERE id=?", now, assets[0].id);
    }
