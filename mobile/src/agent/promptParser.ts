...
export function parsePromptResult(content: string, messageId: string): PromptParseResult | null {
  ...
  if (titled) return { promptText: titled, sourceMessageId: messageId, confidence: 'high' };
  const fenced = content.match(FENCE)?.[1]?.trim();
  if (fenced) return { promptText: fenced, sourceMessageId: messageId, confidence: 'high' };
  const field = content.match(FIELD)?.[1]?.trim();
  if (field) return { promptText: field, sourceMessageId: messageId, confidence: 'medium' };
  return null;
}
